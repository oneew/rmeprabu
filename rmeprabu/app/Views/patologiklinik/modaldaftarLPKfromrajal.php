...
<div id="modaldaftarLPKfromrajal" class="modal fade in" role="dialog" data-backdrop="static" data-keyboard="false" aria-labelledby="fullWidthModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-full-width">
        <div class="modal-content">
            <div class="modal-header">

                <h4 class="modal-title" id="myModalLabel">Form Pendafaran Patologi Klinik(Pasien Rawat Jalan & IGD)</h4>
            </div>

            <?= form_open('RegLPK/simpandatafromrajal', ['class' => 'formperawat']); ?>
            <?= csrf_field(); ?>
                    ...
                    <div class="form-body">
                                ...
                                <div class="form-group">
                                    ...
                                    if ($groups == "IGD") {
                                        ...
                                        if ($groups == "IRJ") {
                                            ...
                                            if ($groups == "ICD") {
                                                $kelas = "ICD";
                                                $jeniskelas = "INSTALASI CENTRAL DIAGNOSTIC";
                                            }
                                        }
                                    }
                        ...
                        <div class="row pt-1">
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    <label class="control-label">Dokter Pemeriksa</label>
                                    <select name="employeenamebaru" id="employeenamebaru" class="select2" style="width: 100%" required>
                                        <option value="">Pilih Dokter Pemeriksa</option>
                                        <?php foreach ($dokterradiologi as $dokterrad) { ?>
                                            <option data-id="<?= $dokterrad['id']; ?>" class="select-employeenamebaru"><?= $dokterrad['name']; ?></option>
                                        <?php } ?>
                                    </select>
                                    <input type="hidden" name="employeebaru" id="employeebaru" value="NONE">
                                    ...
                                    <input type="hidden" id="asalDaftar" name="asalDaftar" class="form-control" value="<?= $groups; ?>" readonly>
                ...
                success: function(response) {
                    if (response.error) {
                        ...
                        Swal.fire({
                            ...
                            if (result.value) {
                                $('#modaldaftarLPKfromrajal').modal('hide');
                                berangkat();
                            }
                        });
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('#employeenamebaru').on('change', function() {
            $.ajax({
                'type': "POST",

                'url': "<?php echo base_url('autocomplete/fill_dokter_penunjang') ?>",
                'data': {
                    key: $('#employeenamebaru option:selected').data('id')
                },
                'success': function(response) {
                    //mengisi value input nama dan lainnya
                    let data = JSON.parse(response);
                    $('#employeenamebaru').val(data.name);
                    $('#employeebaru').val(data.code);
