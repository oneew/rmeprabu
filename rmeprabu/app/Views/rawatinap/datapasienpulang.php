<table id="registerranap" class="tablesaw table-bordered table-hover table no-wrap">
    ...
    <tbody>
        ...
        ?>
            <tr>
                <td>
                    <form method="post" action="<?= base_url(); ?>/PelayananRanap/rincianranap_pasienpulang">
                        <input type="hidden" name="id" id="id" value="<?= $row['id']; ?>">
                        <input type="hidden" name="referencenumber" referencen="referencenumber" value="<?= $row['referencenumber']; ?>">
                ...
                <td>
                    ...
                    <?php if ($row['validation'] == "BELUM") { ?>
                        <button type="button" class="btn waves-effect waves-light btn-rounded  btn-outline-danger btn-sm" onclick="hapusPulang('<?= $row['id']; ?>')"> <i class="fa fa-trash"></i></button>
                    <?php } ?>
    ...
    $(function() {
        ...
        if ('speechSynthesis' in window) {
            ...
            $('.suara').click(function() {
                ...
                msg.pitch = 1;
                msg.text = "nomor antrian " + $(this).data('noantrian') + ", atas nama pasien " + $(this).data('nama') + ", silahkan masuk ke poli " + $(this).data('poli');
...
<script>
    function hapusPulang(id) {

        Swal.fire({
            title: 'Hapus',
            text: "Apakah anda yakin akan membatalkan pulang pasien ?",
            icon: 'warning',
            ...
            if (result.value) {
                $.ajax({
                    type: "post",
                    url: "<?php echo base_url('PelayananRanap/hapusPulangPasien'); ?>",
                    data: {
                        id: id
                    },
                    dataType: "json",
...
<script>
    function SuratKematian(id) {
        $.ajax({
            type: "post",
            url: "<?php echo base_url('PelayananRanap/CreateSuratKematian'); ?>",
            data: {
                id: id
            },
            dataType: "json",
