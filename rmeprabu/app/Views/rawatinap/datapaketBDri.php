    ...
    <tbody>
        ...
        foreach ($tampildata as $row) :
            ...
            <tr>
                ...
                <?= $row['groupname']; ?>|<?= $row['category']; ?>|<?= $row['categoryname']; ?>|<?= $row['price']; ?>|0|<?= $row['price']; ?>|0|<?= $row['price']; ?>|<?= $row['share1']; ?>|<?= $row['share2']; ?>|PELAYANAN DAN TINDAKAN PENUNJANG MEDIS||<?= session()->get('email'); ?>|<?= date('Y-m-d h:m:s'); ?>|<?= $row['kelompokLab']; ?>|<?= $pasienaddress; ?>|<?= $asal_lab; ?>|<?= $jenkel; ?>|<?= $pasiendateofbirth; ?>|<?= $usia; ?>|<?= $icdxname; ?>|<?= $jenisrawat; ?>|<?= $koinsiden; ?>|<?= $goldar; ?>" /></td>
                <td><?= $no ?></td>
                <td><?= $row['code']; ?></td>
                <td><?= $row['name'] ?></td>
                <td><?= $row['price'] ?></td>
