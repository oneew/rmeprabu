            ...
            foreach ($tampildata as $row) :
                ...
                <tr>
                    ...
                    <td class="text-center">
                        ...
                        <button type="button" class="btn waves-effect waves-light btn-rounded btn-outline-info btn-sm btnprintsep" onclick="Cetak('<?= $row['journalnumber']; ?>')"> <i class="fas fa-compress"></i></button>
                    </td>
                    <td><?= $no ?></td>
                    <td><?= $row['documentdate'] ?>
                    ...
                    <td><span class="badge badge-info"><?= $row['paymentmethodname'] ?></span>
                        ...
                        <br><span class="<?php if ($row['grandtotal'] > $row['paymentamount']) {
                    ...
                    <td><?= $row['poliklinikname']; ?>
                        ...
                        <br>TglPelayanan : <?= $row['documentdate']; ?>
                    </td>
...
<script>
    function Cetak(journalnumber) {
        $.ajax({
            type: "post",
            url: "<?php echo base_url('KasirRAD/lihatrincianpenunjang_validasi'); ?>",
            data: {
                journalnumber: journalnumber
            },
            dataType: "json",
            success: function(response) {
                if (response.suksespenunjang) {
                    $('.viewmodal').html(response.suksespenunjang).show();
                    $('#modalpembayaranpenunjang_validasi').modal('show');
