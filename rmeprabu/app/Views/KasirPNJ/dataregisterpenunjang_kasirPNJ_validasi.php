<table id="registerranap" class="tablesaw table-bordered table-hover table no-wrap">
    ...
    <tbody>
        ...
        ?>
            <tr>
                <td>
                    <form method="post" action="KasirRAD/rincianpenunjang_aftervalidasi">
                        <input type="hidden" name="id" id="id" value="<?= $row['journalnumber']; ?>">
                ...
                <td>
                    ...
                    <button type="button" class="btn waves-effect waves-light btn-rounded btn-outline-success btn-sm suara" data-nama="<?= $row['pasienname']; ?>" data-poli="<?= $row['groups']; ?>"> <i class="mdi mdi-volume-high"></i></button>
                </td>
                <td><?= $no ?></td>
                <td><?= $row['groups'] ?></td>
    ...
    $(function() {
        ...
        if ('speechSynthesis' in window) {
            ...
            $('.suara').click(function() {
                ...
                msg.pitch = 1;
                msg.text = "atas nama pasien atau keluarga pasien" + $(this).data('nama') + ", silahkan masuk ke bagian kasir penunjang ";
