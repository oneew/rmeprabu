    ...
    <tbody>
        ...
        ?>
            <tr>
                ...
                <td>
                    <form method="post" action="PelayananBD/inputdetailBD2">
                ...
                <td><span class="<?php if ($row['validation'] == "BELUM") {
                                    ...
                                    }  ?>"><b><?= $row['validation'] ?></b></span></td>

                <td><?= $no ?></td>
                <td><b><?= $row['documentdate'] ?>
                        <br><?= $row['journalnumber'] ?>
                        <br>Reborn :<?= $row['created_at'] ?></b>
                </td>
                <td><b><?= $row['token_radiologi'] ?></b></td>
                <td><b>
                        <h5><?= $row['pasienid'] ?>
                        </h5>
                    </b></td>
                ...
                <td class="center"> <?php
                                    ...
                                    ?></td>
                <td><b><?= $row['paymentmethod'] ?></b></td>
                <td><b><?= $row['doktername'] ?><b></td>
                <td><b><?= $row['pasienaddress'] ?></b></td>
    ...
    function hapusHeader(id) {
        ...
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    type: "post",
                    url: "<?php echo base_url('PelayananBD/hapusHeaderBD'); ?>",
                    data: {
                        id: id
                    },
                    dataType: "json",
