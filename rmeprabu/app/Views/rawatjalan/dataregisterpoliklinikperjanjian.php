<table id="registerranap" class="tablesaw table-bordered table-hover table no-wrap">
    ...
    <tbody>
        ...
        ?>
            <tr>
                <td>
                    <form method="post" action="<?= base_url(); ?>/PelayananRawatJalan/rincianrawatjalan">
                ...
                <td>
                    ...
                    <button type="button" class="btn waves-effect waves-light btn-rounded btn-outline-info btn-sm" onclick="VP('<?= $row['id']; ?>')"> <i class="fas fa-sign-language"></i></button>
                </td>
                ...
                <td><?= $row['poliklinikname'] ?>
                    <br><span class="badge badge-success"><?= $row['doktername'] ?></span>
                </td>
...
<script>
    function VP(id) {
        $.ajax({
            type: "post",
            url: "<?php echo base_url('PelayananRawatJalan/ValidasiFromModal'); ?>",
            data: {
                id: id
            },
            dataType: "json",
            success: function(response) {
                if (response.suksesmodalvalidasi) {
                    $('.viewmodalvalidasi').html(response.suksesmodalvalidasi).show();
                    $('#modalvalidasipoliNew').modal('show');
