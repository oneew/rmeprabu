<table id="datavalidasi" class="tablesaw table-bordered table-hover table no-wrap">
    <thead>
        <tr>
    ...
    <tbody>
        ...
        ?>
            <tr>
                <td>
                    <form method="post" action="KasirRJ/rincianrawatjalan">
                ...
                <td>
                    <button type="button" class="btn waves-effect waves-light btn-rounded btn-outline-info btn-sm" onclick="Bayar('<?= $row['id']; ?>')"> <i class="fas fa-pallet"></i></button>
                    <button type="button" id="btn-card" class="btn waves-effect waves-light btn-rounded  btn-outline-secondary btn-sm btn-cardvalidasi" data-pasiencard="<?= $row['paymentcardnumber']; ?>" data-registerdate="<?= $row['documentdate']; ?>"><span class="mr-1"><i class="fa fa-check"></i></span>Cek</button>
                    <button type="button" class="btn waves-effect waves-light btn-rounded btn-outline-success btn-sm suaravalidasi" data-nama="<?= $row['pasienname']; ?>" data-poli="<?= $row['poliklinikname']; ?>" data-noantrian="<?= $row['noantrian']; ?>"> <i class="mdi mdi-volume-high"></i></button>
                </td>
                <td><?= $no ?></td>
...
<script type="text/javascript">
    ...
    $('.btn-cardvalidasi').on('click', function() {
...
<script>
    function Bayar(id) {
        $.ajax({
            type: "post",
            url: "<?php echo base_url('KasirRJ/BayarRajal'); ?>",
            data: {
                id: id
            },
            dataType: "json",
