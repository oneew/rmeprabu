    ...
    <table id="datapermintaan" class="table color-table success-table" style="font-family: Verdana, Geneva, Tahoma, sans-serif; font-size:small; color:black" cellspacing="0" cellpadding="0">
        ...
        <tbody>
            ...
            foreach ($DetailObat as $row) :
                $no++; ?>
                <tr>
                    <td> <button type="button" class="btn btn-danger btn-sm" onclick="tambahkanvitual('<?= $row['id'] ?>')"> <i class="fa fa-hand-point-right"></i> </button></td>
                    <td><?= $no ?></td>
                    <td><?= $row['code']  ?></td>
                    <td><?= $row['name']  ?></td>
                    <td><?= $row['qty']  ?></td>
...
<script>
    function tambahkanvitual(id) {
        $.ajax({
            type: "post",
            url: "<?php echo base_url('DistribusiAmprahFarmasi/InputDistribusiModalVitual'); ?>",
            data: {
                id: id
            },
            dataType: "json",
            success: function(response) {
                if (response.sukses) {
                    ...
                    $('#modalinputdistribusibaruvitual').modal('show');
