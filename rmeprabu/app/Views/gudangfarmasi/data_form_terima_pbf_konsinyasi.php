        ...
        <tbody>
            ...
            foreach ($DetailObat as $row) :
                ...
                <tr>
                    ...
                    <td><?= $row['tax']  ?></td>
                    <td><?php echo number_format($totalbersih, 2, ",", "."); ?></td>
                    <td><?php echo number_format($row['purchasepricebefore'], 2, ",", "."); ?></td>
                    <td><span class="<?php if ($row['price'] == $row['purchasepricebefore']) {
                                            echo "badge badge-info";
                                            $kata = "Tetap";
                                        } else  if ($row['price'] > $row['purchasepricebefore']) {
                                            echo "badge badge-danger";
                                            $kata = "Naik";
                                        }
                                        if ($row['price'] < $row['purchasepricebefore']) {
    ...
    function hapus(id) {
        ...
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    type: "post",
                    url: "<?php echo base_url('ObatMasukGudangKonsinyasi/hapus_detail_terima_pbf'); ?>",
                    data: {
                        id: id
                    },
                    dataType: "json",
