...
<div id="printThis">
    <div class="pull-center text-center">
        <address>
            <h4>Data Master Alat Kesehatan Habis Pakai</h4>
            </p>
        </address>
    </div>
    <table id="radiologi" class="tablesaw table-bordered table-hover table no-wrap" style="font-family: Verdana, Geneva, Tahoma, sans-serif; font-size:small; color:black" cellspacing="0" cellpadding="0">
        ...
        <tbody>
            ...
            ?>
                <tr>
                    <td><?= $no ?></td>
                    <td><button type="button" class="btn btn-info btn-sm" onclick="updateakhp('<?= $row['id'] ?>')"> <i class="mdi mdi-apple-keyboard-command"></i></button> <?= $row['code'] ?></td>
                    <td><?= $row['name'] ?></td>
                    <td><?= $row['volume'] ?></td>
                    <td><?= $row['uom'] ?></td>
...
<script>
    function updateakhp(id) {
        $.ajax({
            type: "post",
            url: "<?php echo base_url('AKHP/editAKHP'); ?>",
            data: {
                id: id
            },
            dataType: "json",
