    ...
    <table id="dataradiologi" class="table color-table success-table" style="font-family: Verdana, Geneva, Tahoma, sans-serif; font-size:small; color:black" cellspacing="0" cellpadding="0">
        ...
        <tbody>
            ...
            foreach ($DetailObat as $row) :
                $no++; ?>
                <tr>
                    <td><button type="button" class="btn waves-effect waves-light btn-rounded btn-outline-danger btn-sm" onclick="hapuspesan('<?= $row['id']; ?>')"> <i class="fa fa-trash"></i></button></td>
                    <td><?= $no ?></td>
                    <td><?= $row['code']  ?></td>
                    <td><?= $row['name']  ?></td>
                    <td><?= $row['qty']  ?></td>
    ...
    function hapuspesan(id) {
        ...
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    type: "post",
                    url: "<?php echo base_url('AmprahFarmasi/hapus_detail_pesan'); ?>",
                    data: {
                        id: id
                    },
                    dataType: "json",
...
<script type="text/javascript">
    $(document).ready(function() {
        $('.btnprint').on('click', function() {
            ...
            window.open("<?php echo base_url('AmprahFarmasi/printpesan') ?>?page=" + id, "_blank");
