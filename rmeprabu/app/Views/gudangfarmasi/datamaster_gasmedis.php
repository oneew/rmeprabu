    ...
    <table id="radiologi" class="tablesaw table-bordered table-hover table no-wrap" style="font-family: Verdana, Geneva, Tahoma, sans-serif; font-size:small; color:black" cellspacing="0" cellpadding="0">
        ...
        <tbody>
            ...
            ?>
                <tr>
                    <td><?= $no ?></td>
                    <td><button type="button" class="btn btn-info btn-sm" onclick="updategasmedis('<?= $row['id'] ?>')"> <i class="mdi mdi-apple-keyboard-command"></i></button> <?= $row['code'] ?></td>
                    <td><?= $row['name'] ?></td>
                    <td><?= $row['volume'] ?></td>
                    <td><?= $row['uom'] ?></td>
...
<script>
    function updategasmedis(id) {
        $.ajax({
            type: "post",
            url: "<?php echo base_url('GASMEDIS/editGASMEDIS'); ?>",
            data: {
                id: id
            },
            dataType: "json",
