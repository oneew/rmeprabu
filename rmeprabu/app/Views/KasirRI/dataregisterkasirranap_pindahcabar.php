...
<script>
    function Cetak(id) {
        $.ajax({
            type: "post",
            url: "<?php echo base_url('KasirRanap/lihatrincianranap_pindahcabar'); ?>",
            data: {
                id: id,
            ...
            dataType: "json",
            success: function(response) {
                if (response.suksespindahcabar) {
                    $('.viewmodal').html(response.suksespindahcabar).show();
                    $('#modalpembayaranranap_pindahcabar').modal('show');
                }
            }
        });
    }
