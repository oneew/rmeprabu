...
<div id="modalpembayaranranap_validasi" class="modal fade" role="dialog" data-backdrop="static" data-keyboard="false" aria-labelledby="fullWidthModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-full-width">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="fullWidthModalLabel">Menu Update Validasi Pembayaran Rawat Inap</h4>
                            ...
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <div class="mt-4 text-center"> <?php echo "<img src='" . $gambar . "' class='rounded-circle' width='100'>"; ?>
                                        ...
                                        <br>
                                        <?php
                                        foreach ($transaksikasir as $trans) :
                                        ?>

                                            <br><span class="badge badge-info"> No. Trans : <?= $trans['validationnumber']; ?></span>
                                            <br><span class="badge badge-info"> Tgl Validasi : <?= $trans['created_at']; ?></span>
                                            <br><span class="badge badge-info"> Kasir : <?= $trans['createdby']; ?></span>
                                            <br>
                                            <p></p>
                                            <select name="alasanBatal" id="alasanBatal" class="selectpicker mb-3 mr-2" data-style="btn-info">
                                                <option>Pilih Alasan Pembatalan ?</option>
                                                <option value="Perbaikan Rincian">Perbaikan Rincian</option>
                                                <option value="Kesalahan Pilih Pasien">Kesalahan Pilih Pasien</option>
                                            </select>


                                            <button id="print" class="btn btn-danger btn-outline btn btnbatalvalidasi" type="button" onclick="BatalValidasi('<?= $trans['validationnumber'] ?>')"> <span><i class="fas fa-shekel-sign"></i></span> Batalkan Pembayaran ?</button>
                                        <?php endforeach; ?>
                                    </div>
                                        ...
                                        <div class="tab-content">
                                            <div class="tab-pane active" id="profile3" role="tabpanel">
                                                <div class="card-body">
                                                    ...
                                                    <?= form_open('KasirRanap/update_validasipembayaran', ['class' => 'formvalidasibayar']); ?>
                                                    <?= csrf_field(); ?>
...
<script>
    function dataresume() {
        ...
        $.ajax({
            ...
            url: "<?php echo base_url('KasirRanap/resumeGabungValidasi') ?>",
            data: {
...
<script>
    function BatalValidasi(validationnumber) {
        Swal.fire({
            title: 'Batal',
            ...
            if (result.value) {
                $.ajax({
                    type: "post",
                    url: "<?php echo base_url('KasirRanap/BatalValidasi'); ?>",
                    data: {
...
<script type="text/javascript">
    function berangkat() {
        window.location.href = "<?php echo base_url('KasirRanap/AfterValidasi'); ?>";
    }
</script>
...
<script>
    $('.filter-input').on('change', function() {
        ...
        $.ajax({
            type: "post",
            url: "<?php echo base_url('KasirRanap/resumeGabungValidasiPilihan') ?>",
            dataType: "json",
            data: {
...
<script>
    $('.filter-koinsiden').on('change', function() {
        ...
        $.ajax({
            type: "post",
            url: "<?php echo base_url('KasirRanap/resumeGabungPilihanKoinsidenValidasi') ?>",
            dataType: "json",
            data: {
                koinsiden: koinsiden,
                pilihancabar: rincian,
