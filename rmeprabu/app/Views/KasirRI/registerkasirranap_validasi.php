                            ...
                            <div class="form-group">
                                ...
                                <select name="poli" id="poli" class="select2 filter-input" style="width: 100%">
                                    ...
                                    <?php foreach ($poli as $p) : ?>
                                        <option value="<?php echo $p['roomname']; ?>"><?php echo $p['roomname']; ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                        </div>
...
<div class="viewmodalvalidasiranap" style="display:none;"></div>

<script>
    function dataRegisterPoli() {
        $.ajax({

            url: "<?php echo base_url('KasirRanap/ambildataAfterValidasi') ?>",
            dataType: "json",
    ...
    $('.filter-input').on('input apply.daterangepicker', function() {
        ...
        $.ajax({
            type: "post",
            url: "<?php echo base_url('KasirRanap/cariAfterValidasi') ?>",
            dataType: "json",
            data: {
