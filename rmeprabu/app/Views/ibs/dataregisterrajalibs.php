            ...
            foreach ($tampildata as $row) :
                ...
                <tr>
                    <td>
                        ...
                        <button type="button" class="btn waves-effect waves-light btn-rounded btn-outline-danger btn-sm" onclick="daftaribsrajal('<?= $row['id']; ?>')"> <i class="far fa-edit"></i></button>
...
<script>
    function daftaribsrajal(id) {
        $.ajax({
            type: "post",
            url: "<?php echo base_url('rawatinap/DaftarIBSRajal'); ?>",
            data: {
                id: id
            },
            dataType: "json",
            success: function(response) {
                if (response.sukses) {
                    $('.viewmodal').html(response.sukses).show();
                    $('#modaldaftaribsrajal').modal('show');
