...
<div id="modaldaftarhdfromrajal" class="modal fade in" role="dialog" data-backdrop="static" data-keyboard="false" aria-labelledby="fullWidthModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-full-width">
        <div class="modal-content">
            <div class="modal-header">

                <h4 class="modal-title" id="myModalLabel">Form Pendafaran Hemodialisa (Pasien Rawat Jalan & IGD)</h4>
            </div>

            <?= form_open('RegHD/simpandatafromrajal', ['class' => 'formperawat']); ?>
            <?= csrf_field(); ?>
                        ...
                        <div class="row pt-1">
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    ?>
                                    <input type="hidden" id="token_radiologirajaligd" name="token_radiologirajaligd" class="form-control" value="<?= $token; ?>">
                                </div>
                            </div>
                            <!--/span-->
                            <div class="col-md-3">
                        ...
                        <div class="row pt-1">
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <input type="date" id="datepicker-autoclose" autocomplete="off" name="tgl_order" class="form-control" value="<?= date('Y-m-d'); ?>">
                                    ...
                                    <input type="hidden" id="locationcode" name="locationcode" class="form-control" value="HD" readonly>
                                    <input type="hidden" id="locationname" name="locationname" class="form-control" value="HEMODIALISA" readonly>
                                    ...
                                    <input type="hidden" id="groups" name="groups" class="form-control" value="HD" readonly>
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    <label class="control-label">Dokter Pemeriksa Hemodialisa</label>
                                    <select name="employeename" id="employeename" class="select2 employeenamerajal" style="width: 100%" required>
                                        <option value="">Pilih Dokter</option>
                                        <?php foreach ($listDokterHD as $HD) { ?>
                                            <option data-id="<?= $HD['id']; ?>" class="select-employeenamerajal"><?= $HD['name']; ?></option>
                                        <?php } ?>
                                    </select>
                                    <input type="text" name="employee" id="employee" class="form-control employeerajal">
...
<script type="text/javascript">
    function berangkat() {
        var page = document.getElementById("token_radiologirajaligd").value;

        window.location.href = "<?php echo base_url('PelayananHemodialisa/inputdetailHD'); ?>?page=" + page;
    }
</script>
                ...
                success: function(response) {
                    if (response.error) {
                        ...
                        Swal.fire({
                            ...
                            if (result.value) {
                                $('#modalorderdaftarradiologi').modal('hide');
                                berangkat();
                            }
                        });
...
<script type="text/javascript">
    $(document).ready(function() {
        $('.employeenamerajal').on('change', function() {
            $.ajax({
                'type': "POST",
                'url': "<?php echo base_url('autocomplete/fill_dokter_HD') ?>",
                'data': {
                    key: $('.employeenamerajal option:selected').data('id')
                },
                'success': function(response) {
                    let data = JSON.parse(response);
                    $('.employeenamerajal').val(data.name);
                    $('.employeerajal').val(data.code);
                }
            })
        })
    });
