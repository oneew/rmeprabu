<div id="modalrmerajal_poliklinik_mobilisasi_dana" class="modal fade in" role="dialog" data-backdrop="static" data-keyboard="false" aria-labelledby="fullWidthModalLabel" aria-hidden="true">
                        ...
                        <div class="card">
                            <div class="card-body">
                                <div class="text-center mb-2">
                                    <button class="btn btn-dark" type="button" onclick="codingDiagnosa('<?= $referencenumber ?>')"><span class="mr-1"><i class="fas fa-plus"></i></span>Kodifikasi Diagnosa</button>
                                </div>
                                <div class="text-center mb-2">
                                    <?php if ($verifikasidiagnosarajal == 0) { ?>
                                        <button id="print" class="btn btn-success btn-outline btn btnbatalperiksa" type="button" onclick="VerifikasiDiagnosarajal('<?= $id ?>')"> <span><i class="fas fa-shekel-sign"></i></span> Simpan Diagnosa ?</button>
                                    <?php } ?>
                                    <?php if ($verifikasidiagnosarajal != 0) { ?>
                                        <button id="print" class="btn btn-danger btn-outline btn btnupdatesep" type="button" onclick="BatalDiagnosarajal('<?= $id ?>')"> <span><i class="fas fa-calendar-check"></i></span> Batal Diagnosa ? </button>
                                    <?php } ?>
                                </div>
                                <div class="text-center mb-2">
                    ...
                    <div class="col-lg-10 col-md-12">
                        <div class="card">
                            <ul class="nav nav-tabs profile-tab" role="tablist">
                                <li class="nav-item"> <a class="nav-link  active" data-toggle="tab" href="#profile3" role="tab">Arsip Digital Pelayanan Tanggal <?php echo tgl_indo($tanggal); ?> Ke <?= $roomname; ?> [<?= $doktername; ?>]</a></li>
                            </ul>
...
<script>
    ...
    function dataresume() {
        $.ajax({
            url: "<?php echo base_url('PelayananRawatJalanRME/ArsipDigitalPoli') ?>",
            data: {
...
<script>
    function VerifikasiDiagnosarajal(id) {
        Swal.fire({
            title: 'Batal',
            ...
            if (result.value) {
                $.ajax({
                    type: "post",
                    url: "<?php echo base_url('PelayananRawatJalanRME/VerifikasiDiagnosarajal'); ?>",
                    data: {
                        id: id,
...
<script>
    function BatalDiagnosarajal(id) {
        Swal.fire({
            title: 'Batal',
            text: "Apakah anda yakin akan membatalkan Verifikasi Sebelumnya ?",
            icon: 'warning',
            ...
            if (result.value) {
                $.ajax({
                    type: "post",
                    url: "<?php echo base_url('PelayananRawatJalanRME/BatalDiagnosarajal'); ?>",
                    data: {
                        id: id,
                        verifikasidiagnosarajal: $('#verifikasidiagnosarajal').val()
