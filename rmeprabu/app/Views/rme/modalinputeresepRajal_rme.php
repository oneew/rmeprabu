<div id="modalinputeresepRajal_rme" class="modal fade in" role="dialog" data-backdrop="static" data-keyboard="false" aria-labelledby="fullWidthModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-scrollable modal-full-width">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="myLargeModalLabel">Resep Elektronik (<?= $relation . ' | ' . $relationname; ?>)</h4>
                    ...
                    <div class="col-lg-12 col-md-12">
                        <div class="card">
                            <ul class="nav nav-tabs profile-tab" role="tablist">
                                ...
                                <li class="nav-item">
                                    <a class="nav-link btn-history-racikan" data-toggle="tab" role="tab">Riwayat Obat Racikan</a>
                                </li>
                            </ul>
...
<script>
    $("body").on("click", "#rowAdder", function() {
        newRowAdd =
            ...
            '<label class="control-label">Keterangan</label>' +
            '<select name="racikan[]" id="racikan2" class="form-control">' +
            <?php foreach ($racikan as $SP) : ?> '<option value="<?= $SP['kode']; ?>" data-name="<?= $SP['name']; ?>" class="select-statuspulang"><?= $SP['name']; ?></option>' +
            ...
            '<select name="koderacikan[]" id="koderacikan" class="form-control">' +
            '<option value="-">-</option>' +
            <?php foreach ($itemracikan as $APS) : ?> '<option data-id="<?= $APS['id']; ?>" data-room="<?= $APS['name']; ?>" class="select-koderacikan"><?= $APS['name']; ?></option>' +
            <?php endforeach; ?> '</select>' +
            '</div>' +
            '<div class="col-md-2">' +
            '<div class="form-group mb-0">' +
    ...
    var list_cara_pakat = [
        <?php foreach ($carapakai as $cara) : ?> "<?= $cara['name']; ?>",
        <?php endforeach; ?>
    ];
    ...
    $('.btn-history-racikan').click(function() {
        $.ajax({
            url: "<?= base_url('PelayananRawatJalanRME/getDataHistoryEresepRacikan') ?>",
            data: {
                pasienid: $('#relation').val()
            },
            dataType: "json",
            success: function(response) {
                $('.viewmodaltemplet').html(response.success).show();
                $('#modal_history_e_resep_racikan').modal({
                    show: true,
                    backdrop: false
                });
            }
