    ...
    <tbody>
        ...
        <?php foreach ($tampildata as $no => $row) : ?>
            <tr>
                <td><?= ++$no; ?></td>
                <td>
                    <button type="button" class="btn btn-success btn-sm btn-rounded" onclick="LihatRME('<?= $row['id'] ?>')"> <i class="fas fa-file-medical"></i></button>
                </td>
                <td>
                    <span class="badge badge-<?= $row['verifikasimobdan'] == 0 ? 'danger' : 'success' ;?>">
                    <?= $row['verifikasimobdan'] == 0 ? 'Belum' : 'Sudah' ;?>
                    </span>
                    <br>
...
<script>
    ...
    function LihatRME(id) {
        $.ajax({
            type: "post",
            url: "<?php echo base_url('PelayananRawatJalanRME/entriRMEMobilisasiDana'); ?>",
            data: {
                id: id
            },
            dataType: "json",
            success: function(response) {
                if (response.sukses) {
                    ...
                    $('#modalrmerajal_poliklinik_mobilisasi_dana').modal('show');
