        ...
        <div class="col">
            <div class="card mx-3">
                ...
                <ul class="list-group list-group-flush">
                    <?php if (get_data_penunjang($data_classroom, $row['name'], 'RAD')) : ?>
                        <?php foreach (get_data_penunjang($data_classroom, $row['name'], 'RAD') as $item) : ?>
                            <li class="list-group-item filter">
                                <div class="form-check px-0">
                                    <input class="tandai-<?= str_replace([' ', '(', ')'], '_', $row['name']); ?>" type="checkbox" id="defaultCheck<?= $item['id']; ?>" name="tandai[]" value="<?= $groups; ?>|<?= $journalnumber; ?>|<?= $documentdate; ?>|<?= $pasienid; ?>|<?= $pasienname; ?>|<?= $paymentmethod; ?>|<?= $paymentmethod; ?>|<?= $classroom; ?>|<?= $classroomname; ?>|<?= $room; ?>|<?= $roomname; ?>|<?= $smf; ?>|<?= $smfname; ?>|<?= $dokter; ?>|<?= $doktername; ?>|<?= $employee; ?>|<?= $employeename; ?>|<?= $asal_lab; ?>|<?= $referencenumber; ?>|<?= $referencenumberparent; ?>|<?= $locationcode; ?>|<?= $item['code']; ?>|<?= $item['name']; ?>|1|<?= $item['groups']; ?>|<?= $item['groupname']; ?>|<?= $item['category']; ?>|<?= $item['categoryname']; ?>|<?= $item['price']; ?>|0|<?= $item['price']; ?>|0|<?= $item['price']; ?>|<?= $item['share1']; ?>|<?= $item['share2']; ?>|PELAYANAN DAN TINDAKAN PENUNJANG MEDIS||<?= session()->get('email'); ?>|<?= date('Y-m-d h:m:s'); ?>|<?= $item['kelompokLab']; ?>|<?= $pasienaddress; ?>|<?= $asal_lab; ?>|<?= $jenkel; ?>|<?= $pasiendateofbirth; ?>|<?= $usia; ?>|<?= $icdxname; ?>|<?= $jns_rawat; ?>|<?= $koinsiden; ?>">
