<div id="modalpilihaskep" class="modal fade" id="bs-example-modal-lg" role="dialog" data-backdrop="static" data-keyboard="false" aria-labelledby="myLargeModalLabel" aria-hidden="true">
            ...
            <div class="modal-body">
                <div class="table-responsive">
                    <table id="datapaketLab" class="tablesaw table-bordered table-hover table no-wrap" width="100%" style="font-size: larger;">
                        ...
                        <tbody>
                            <?php foreach ($list_askep as $no => $row) : ?>
                                <tr>
                                    <td><?= ++$no ?></td>
                                    <td>
                                        <input type="checkbox" id="tandai" name="tandai[]" value="<?= $row['rencana']; ?>|<?= $row['diagnosa']; ?>" />
                                    </td>
                                    <td><?= $row['rencana'] ?></td>
                                    <td><?= $row['otec'] ?></td>
                    ...
                    if (response.sukses) {
                        Swal.fire({
                            ...
                            if (result.value) {
                                ...
                                // historiradiologi();
                                // resumeexpertise();
                                $('#modalpilihaskep').modal('hide');
                                $('#mymce2').html(response.rencana_askep);
