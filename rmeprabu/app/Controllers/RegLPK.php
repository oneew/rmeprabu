...
class RegLPK extends BaseController
{
    ...
    public function order()
    {
        if ($this->request->isAJAX()) {
            ...
            $data = [
                ...
                'dokterradiologi' => $this->_data_dokter_patologiklinik(),

            ];
            $msg = [
                'sukses' => view('patologiklinik/modaldaftarpk', $data)
            ];
            echo json_encode($msg);
        }
    }
    ...
    private function _data_dokter_patologiklinik()
    {
        $request = Services::request();
        $m_auto = new Model_autocomplete($request);
        $list = $m_auto->get_list_dokter_pk();
        return $list;
    }
    ...
    public function orderfromrajal()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = [
                'sukses' => view('patologiklinik/modaldaftarLPKfromrajal', $data)
            ];
            echo json_encode($msg);
        }
    }
    ...
    public function simpandata()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$valid) {
                ...
                $query = $db->query("SELECT MAX(journalnumber) as kode_jurnal,  MAX(numberseq)as antrian FROM transaksi_pelayanan_penunjang_header WHERE  documentdate='$documentdate' AND groups='LPK' LIMIT 1");
                ...
                helper('text');
                $tokenpenunjang = random_string('alnum', 8);
                $token = strtoupper($tokenpenunjang);

                $newkode = $visited . $underscore . $lokasi . $underscore . $pasienid . $underscore . $token . $underscore . $today . $underscore . sprintf('%06s', $nourut);
                //$newkode = $visited . $underscore . $lokasi . $underscore . $pasienid . $underscore . $today . $underscore . sprintf('%06s', $nourut);
                $numberseq = $antrian + 1;
                ...
                $simpandata = [
                    ...
                    'employee' => $this->request->getVar('employeebaru'),
                    'employeename' => $this->request->getVar('employeenamebaru'),
                    ...
                    //'asalDaftar' => $this->request->getPost('asalDaftar'),
                    'numberseq' => $numberseq,
                ...
                $msg = [
                    'sukses' => 'Pendaftaran Patologi Klinik Sudah Berhasil Dilakukan'
                ];
            }
            echo json_encode($msg);
        } else {
    ...
    public function simpandatafromrajal()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$valid) {
                ...
                $newkode = $new_era . $underscore . $visited . $underscore . $lokasi . $underscore . $pasienid . $underscore . $token . $underscore . $today . $underscore . sprintf('%06s', $nourut);
                $numberseq = $antrian + 1;
                ...
                $perawat->insert($simpandata);
                $msg = [
                    'sukses' => 'Pendaftaran Laboratorium Patologi Klinik Sudah Berhasil Dilakukan'
                ];
            }
            echo json_encode($msg);
        } else {
    ...
    public function simpandataNonRM()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$valid) {
                ...
                $query = $db->query("SELECT MAX(journalnumber) as kode_jurnal, MAX(numberseq)as antrian FROM transaksi_pelayanan_penunjang_header WHERE  documentdate='$documentdate' AND groups='LPK' AND types='NRM' LIMIT 1");
                ...
                //$newkode = $new_era . $underscore . $visited . $underscore . $lokasi . $underscore . $today . $underscore . sprintf('%06s', $nourut);
                ...
                $rujukanParsial = $this->request->getVar('rujukanParsial');
                if ($rujukanParsial == 1) {
                    $rujukanParsial = 1;
                } else {
                    $rujukanParsial = 0;
                ...
                $simpandata = [
                    ...
                    'orderpemeriksaan' => $this->request->getPost('orderpemeriksaan'),
                    'tgl_order' => $tgl_order,
                    'token_radiologi' => $this->request->getPost('token_lpa'),
                    'memo' => $this->request->getPost('memo'),
                    'note' => $this->request->getPost('note'),
                    'numberseq' => $numberseq,
                    //'status' => $this->request->getPost('status'),
                    //  'asalDaftar' => $this->request->getPost('asalDaftar'),
                    //'rujukanParsial' => $rujukanParsial,

                ];
                $perawat = new ModelDaftarRadiologiNonRM;
                $perawat->insert($simpandata);
                $msg = [
                    'sukses' => 'Pendaftaran Patologi Klinik Pasien Datang langsung Sudah Berhasil Dilakukan'
                ];
            }
            echo json_encode($msg);
        } else {
