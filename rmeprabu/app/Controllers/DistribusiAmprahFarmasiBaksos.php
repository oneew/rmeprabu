...
use App\Models\ModelDistribusiHeaderBaksos;
use App\Models\ModelDistribusiDetailBaksos;
...
class DistribusiAmprahFarmasiBaksos extends BaseController
{
    ...
    public function index()
    {
        ...
        echo view('gudangfarmasi/form_distribusi_amprah_barang_baksos', $data);
    }
    ...
    public function simpandatadistribusi()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$valid) {
                ...
                $query = $db->query("SELECT MAX(journalnumber) as kode_jurnal, MAX(numberseq)as noantrian FROM transaksi_farmasi_baksos_gudang_header WHERE documentyear='$tahun' AND locationcode='$locationcode' and reborn=1 LIMIT 1");
                ...
                $perawat = new ModelDistribusiHeaderBaksos();
        ...
        if ($this->request->isAJAX()) {
            ...
            if (!$valid) {
                ...
                if ($jumlah > $stock_gudang) {
                    ...
                    $perawat = new ModelDistribusiDetailBaksos;
                    $perawat->insert($simpandata);
                    $msg = [
    ...
    public function resumeDistribusi()
    {
        if ($this->request->isAJAX()) {
            ...
            $journalnumber = $this->request->getVar('journalnumber');
            $data = [
                'DetailObat' => $resume->search_detail_distribusi_baksos($journalnumber)
            ];
            $msg = [
                'data' => view('gudangfarmasi/data_form_distribusi_baksos', $data)
            ];
            echo json_encode($msg);
        } else {
            exit('tidak dapat diproses');
    ...
    public function DDA()
    {
        ...
        return view('gudangfarmasi/registerDDABaksos', $data);
    }
    ...
    public function ambildataDDA()
    {
        if ($this->request->isAJAX()) {
            ...
            $data = [
                'tampildata' => $register->ambildataDDABaksos()
            ];
            $msg = [
                'data' => view('gudangfarmasi/dataregisterDDABaksos', $data)
            ];
            echo json_encode($msg);
        } else {
            exit('tidak dapat diproses');
    ...
    public function caridataDDA()
    {
        if ($this->request->isAJAX()) {
            ...
            $register = new ModelDepoSPHeader();
            $data = [
                'tampildata' => $register->search_RegisterDDABaksos($search)
            ];
    ...
    public function printdistribusi()
    {
        ...
        $data = [
            'dataopname' => $pasien->Distribusiheaderbaksos($id),
            'header1' => $row2['header1'],
            'header2' => $row2['header2'],
            'status' => $row2['status'],
            'alamat' => $row2['alamat'],
            'deskripsi' => $row2['deskripsi'],
            'tampildata' => $pasien->Distribusidetailbaksos($id),
        ];

        $html = view('pdf/stylebootstrap');
        $html .= view('pdf/distribusi_amprah_pdf', $data);
    ...
    public function DetailDDA2($id = '')
    {
        ...
        $m_icd = new ModelPasienRanap($this->request);
        $row = $m_icd->get_data_DDABaksos($id);
        $data = [
            'id' => $row['id'],
            'journalnumber' => $row['journalnumber'],
        ...
        return view('gudangfarmasi/form_distribusi_amprah_barang_baksos_add', $data);
    }
    ...
    public function printdistribusiharga()
    {
        $dompdf = new Dompdf();
        ...
        $html = view('pdf/stylebootstrap');
        $html .= view('pdf/distribusi_amprah_pdf_harga', $data);
