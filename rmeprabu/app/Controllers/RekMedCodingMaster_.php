...
use App\Models\ModelCodingMaster;
use Config\Services;
use CodeIgniter\HTTP\Request;

use Dompdf\Dompdf;

class RekMedCodingMaster extends BaseController
{
    ...
    public function index()
    {
        ...
        return view('rekammedik/registerpoliklinik_coding_master', $data);
    }
    ...
    public function caridataregisterpoli()
    {
        if ($this->request->isAJAX()) {
            ...
            $dateout = explode('-', $search['created_at']);
    ...
    public function LapCoding()
    {
        ...
        return view('rekammedik/registerpoliklinik_codingmaster_lapcoding', $data);
    }
    ...
    public function ambildataLapCoding()
    {
        if ($this->request->isAJAX()) {
            ...
            $register = new ModelCodingMaster();
            $data = [
                // 'tampildata' => $register->ambildatarajal_lapcoding()
                'tampildata' => $register->ambildatarajal_lapcoding_aliit()
            ];

            $msg = [
                'data' => view('rekammedik/dataregisterpoliklinik_codingmaster_lapcoding', $data)
            ];
            echo json_encode($msg);
        } else {
            exit('tidak dapat diproses');
