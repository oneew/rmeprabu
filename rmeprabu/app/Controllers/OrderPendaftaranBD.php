...
class OrderPendaftaranBD extends BaseController
{
    ...
    public function order()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = [
                'sukses' => view('rawatinap/modalorderdaftarbd', $data)
            ];
            echo json_encode($msg);
        }
    }
    ...
    public function simpandata()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$valid) {
                ...
                $query = $db->query("SELECT MAX(journalnumber) as kode_jurnal FROM transaksi_pelayanan_penunjang_header WHERE  documentdate='$documentdate' AND groups='BD' LIMIT 1");
                ...
                helper('text');
                $tokenpenunjang = random_string('alnum', 4);
                $token = strtoupper($tokenpenunjang);
                $pasienid = $this->request->getVar('pasienid');
                
                $newkode = $visited . $underscore . $lokasi . $pasienid . $underscore . $token . $underscore . $today . $underscore . sprintf('%06s', $nourut);
                ...
                $msg = [
                    'sukses' => 'Order Pelayanan Laboratorium Bank Darah Sudah Dikirimkan',
                    'JN' => $newkode,
                ];
            }
            echo json_encode($msg);
    ...
    private function _data_pa()
    {
        $request = Services::request();
        $m_auto = new Model_autocomplete($request);
        $list = $m_auto->get_list_pelayanan_BD();
        return $list;
    }
    ...
    public function orderbdrajal()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = [
                'sukses' => view('rawatjalan/modalorderdaftarbd_rajal', $data)
            ];
            echo json_encode($msg);
        }
    }
    ...
    public function simpandatabdrajal()
    {
        if ($this->request->isAJAX()) {
            $validation = \Config\Services::validation();
            $valid = $this->validate([
            ...
            if (!$valid) {
                ...
                helper('text');
                $tokenpenunjang = random_string('alnum', 3);
                $token = strtoupper($tokenpenunjang);
                $pasienid = $this->request->getVar('pasienid');
                

                $newkode = $visited . $underscore . $lokasi . $token . $pasienid . $underscore . $today . $underscore . sprintf('%06s', $nourut);
                // $orderpemeriksaan = implode(",", $this->request->getPost('orderpemeriksaan'));
    ...
    public function orderbdigd()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = [
                'sukses' => view('igd/modalorderdaftarbd_igd', $data)
            ];
            echo json_encode($msg);
        }
    }
    ...
    public function simpandatabdigd()
    {
        if ($this->request->isAJAX()) {
            $validation = \Config\Services::validation();
            $valid = $this->validate([
            ...
            if (!$valid) {
                ...
                helper('text');
                $tokenpenunjang = random_string('alnum', 9);
                $token = strtoupper($tokenpenunjang);
                $pasienid = $this->request->getVar('pasienid');

                $newkode = $visited . $token . $pasienid . $underscore . $lokasi . $underscore . $today . $underscore . sprintf('%06s', $nourut);
                // $orderpemeriksaan = implode(",", $this->request->getPost('orderpemeriksaan'));
