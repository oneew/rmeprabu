...
class PendaftaranRanap extends BaseController
{
    ...
    public function BatalPeriksa()
    {
        if ($this->request->isAJAX()) {
            ...
            $statuspasien = 'PULANG (BEROBAT JALAN)';
    ...
    public function formedit()
    {
        if ($this->request->isAJAX()) {
            ...
            $data = [
                ...
                'bed' => $this->bed(),
                'namasmf' => $this->smf_real(),
                'cabar' => $this->data_payment(),
                'Hakkelas' => $this->kelasrawatreal(),
                'pasienlama' => $m_icd->get_data_rajal($id),


            ];
            $msg = [
                'sukses' => view('rawatinap/modaldaftarRanapNew', $data)
            ];
            echo json_encode($msg);
        }
    }
    ...
    public function simpandata()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$valid) {
                ...
                $tglspr1 = $this->request->getVar("tglspr");

                $mulai = str_replace('/', '-', $tglspr1);
                $tglspr = date('Y-m-d', strtotime($mulai));
                ...
                $masuk = $this->request->getVar('datetimein');
                $mulaimasuk = str_replace('/', '-', $masuk);
                $documentdate = date('Y-m-d', strtotime($mulaimasuk));
                //$documentdate = $this->request->getVar('datetimein');


                $jammasuk = $this->request->getVar('jammasuk');
                $datetimein = $documentdate . ' ' . $jammasuk;
                $query = $db->query("SELECT MAX(journalnumber) as kode_jurnal FROM transaksi_pelayanan_daftar_rawatinap WHERE  documentdate='$documentdate' and types='BARU' LIMIT 1");
                ...
                $newkode = $lokasi . $underscore . $groups . $underscore  . $token . $today . $underscore . sprintf('%06s', $nourut);
                $pasienid = $this->request->getVar('pasienid');
                $cekregister = new ModelDaftarRanap;
                $hasilcek = $cekregister->cek_register_today($pasienid);
                $cekdulu = isset($hasilcek['pasienid']) != null ? $hasilcek['pasienid'] : "";


                $kelaspasien = $this->request->getVar('pasienclassroom');
                $smfpasien = $this->request->getVar('smfname');
                $ruanganranap = $this->request->getVar('roomname');
                $bedranap = $this->request->getVar('bednumber');


                if ($kelaspasien == "") {
                    $msg = [
                        'gagal' => true,
                        'pesan' => 'Pilih Hak Kelas Perawatan Terlebih Dahulu!'
                    ];
                } else 
                    if ($cekdulu != "") {
                    $msg = [
                        'gagal' => true,
                        'pesan' => 'Pasien Tersebut Masih ada Pada Data Pasien Yang Sedang Dirawat, Pasien Tidak Dapat Didaftarkan !'
                    ];
                } else 
                if ($smfpasien == "") {
                    $msg = [
                        'gagal' => true,
                        'pesan' => 'Pilih SMF Perawatan Terlebih Dahulu!'
                    ];
                } else 
                if ($ruanganranap == null) {
                    $msg = [
                        'gagal' => true,
                        'pesan' => 'Pilih Ruangan Ranap Terlebih Dahulu!'
                    ];
                } else 
                if ($bedranap == null) {
                    $msg = [
                        'gagal' => true,
                        'pesan' => 'Pilih Nomor Bed Terlebih Dahulu!'
                    ];
                } else {
                    $simpandata = [
    ...
    public function kelasrawatreal()
    {

        $m_auto = new Modelrajal();
        $list = $m_auto->get_list_kelas_real();
        return $list;
    }
    ...
    public function ajax_pulang()
    {
        $request = Services::request();
        $keterangan = $request->getPost('keterangan');
        $m_combo_pulang = new Modelrajal();
        $list['name'] = $m_combo_pulang->get_pulang($keterangan);

        echo json_encode($list['name']);
    }


    public function metode_pembayaran_kasir()
    {
        $request = Services::request();
        $metodepembayaran = $request->getPost('metodepembayaran');
        $m_combo_pulang = new Modelrajal();
        $list['namabank'] = $m_combo_pulang->get_list_bank($metodepembayaran);

        echo json_encode($list['namabank']);
    }
    ...
    public function ajax_racikan()
    {
        $request = Services::request();
        $keterangan = $request->getPost('keterangan');
        $m_combo_pulang = new Modelrajal();
        $list['name'] = $m_combo_pulang->get_racikan($keterangan);

        echo json_encode($list['name']);
    }

    public function historipiutang()
    {
        if ($this->request->isAJAX()) {

            $pasienid = $this->request->getVar('pasienid');

            $piutang = new Modelrajal();
            $datapiutang = $piutang->get_piutang($pasienid);
            $datapiutangrajal = $piutang->get_piutang_rajal($pasienid);
            $data = [
                'piutang' => $datapiutang,
                'piutangrajal' => $datapiutangrajal,

            ];
            $msg = [
                'suksespiutang' => view('rawatinap/modalhistoripiutang', $data)
            ];
            echo json_encode($msg);
        }
    }
    ...
    public function HistoriPasienLama()
    {
        if ($this->request->isAJAX()) {
            ...
            $data = [
                ...
                'pasienlama' => $m_icd->get_data_pasien_lama_master($id),
