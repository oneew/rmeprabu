...
class AmprahFarmasiRuangan extends BaseController
{
    ...
    public function index()
    {
        ...
        echo view('rawatinap/form_amprah_barang_ruangan', $data);
    }
    ...
    public function simpandataamprah()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$valid) {
                ...
                $query = $db->query("SELECT MAX(journalnumber) as kode_jurnal, MAX(numberseq)as noantrian FROM transaksi_farmasi_deposp_header WHERE groups='$groups' AND documentyear='$tahun' AND locationcode='$locationcode' LIMIT 1");
    ...
    public function caridataobat()
    {
        if ($this->request->isAJAX()) {
            ...
            $data_obat = $gudang->search_DataObat_amprah_ruangan($search);
            $data_obat_kosong = $gudang->search_DataObat_amprah_ruangan_farmasi($search);

            if ($data_obat == null) {
                $data_obat = $data_obat_kosong;
                $tanda = 0;
            } else {
                $data_obat = $data_obat;
                $tanda = 1;
            ...
            // var_dump($data_obat);
            // var_dump($data_obat_kosong);
            // die();

            $data = [
                'tampildata' => $data_obat,
                'locationcode' => $locationcode,
                'tanda' => $tanda,
            ];
    ...
    public function detailobat()
    {
        if ($this->request->isAJAX()) {
            ...
            $locationcode = $this->request->getVar('locationcode');
            $tanda = $this->request->getVar('tanda');


            $m_icd = new ModelMasterObat();
            //$row = $m_icd->get_data_obat_depo_sp_ruangan($code, $locationcode);
            $row = $m_icd->get_data_obat_depo_sp_ruangan_baru($code, $locationcode, $tanda);
    ...
    public function DSP()
    {
        ...
        return view('rawatinap/registerDSPruangan', $data);
    }
    ...
    public function ambildataDSP()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = [
                'data' => view('rawatinap/dataregisterDSPruangan', $data)
            ];
            echo json_encode($msg);
        } else {
            exit('tidak dapat diproses');
    ...
    public function DetailDSP($id = '')
    {
        ...
        return view('rawatinap/form_amprah_barang_add', $data);
    }
    ...
    public function Distribusi()
    {
        ...
        return view('rawatinap/laporan_distribusi_ruangan', $data);
    }
    ...
    public function ambildataDistribusi()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = [
                'data' => view('rawatinap/data_laporan_distribusi_ruangan', $data)
            ];
            echo json_encode($msg);
        } else {
            exit('tidak dapat diproses');
