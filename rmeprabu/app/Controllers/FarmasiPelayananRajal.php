...
class FarmasiPelayananRajal extends BaseController
{
    ...
    public function printetiketSemua()
    {
        ...
        return view('cetakan/etiketfarmasirajalsemua', $data);
    }
    ...
    public function Search_Obat_Pelayanan_eresep()
    {
        if ($this->request->isAJAX()) {
            ...
            $gudang = new ModelMasterObat();
            $lokasi = $this->request->getVar('locationcode');
    ...
    public function caridataobateresep()
    {
        if ($this->request->isAJAX()) {
            ...
            $gudang = new ModelMasterObat();
            $data = [
                'tampildata' => $gudang->search_DataObat_pelayanan_baru($search),
    ...
    public function orderesepranap()
    {
        if ($this->request->isAJAX()) {
            ...
            $data = [
                ...
                'carapakai' => $resume->cara_pakai(),
                'carapetunjuk' => $resume->cara_petunjuk(),
                'kelompokdepo' => $resume->kelompok_depo(),

            ];
            $msg = [
                'sukses' => view('rawatinap/modaleresep_ranap', $data)
            ];
            echo json_encode($msg);
        }
    }
    ...
    public function simpandata_eresepranap()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$valid) {
                ...
                //$query = $db->query("SELECT journalnumber as kode_jurnal, numberseq as noantrian FROM transaksi_farmasi_pelayanan_header WHERE documentyear='$tahun'  AND documentmonth='$bulan'  AND locationcode='$locationcode' and tanda=2 ORDER BY id desc LIMIT 1");
                $kodedepo = $this->request->getVar('kodedepo');
                $query = $db->query("SELECT journalnumber as kode_jurnal, numberseq as noantrian FROM transaksi_farmasi_pelayanan_header WHERE groups='RI' and locationcode='$kodedepo' and documentyear='$tahun'  and documentmonth='$bulan' ORDER BY id desc LIMIT 1");
                ...
                $tanda = 'RRI';
                $today = date('ymd', strtotime($documentdate));
                $waktu = $bulan . $potongtahun;

                //$newkode = $tanda . $underscore . $pasienid . $underscore . $locationcode . $underscore . $today . $underscore . sprintf('%06s', $nourut);
                $newkode = $tanda . $underscore . $kodedepo . $underscore . $waktu . $underscore . sprintf('%06s', $nourut);
                // var_dump($potongtahun);
                // var_dump($bulan);
                // var_dump($newkode);
                // die();
    ...
    public function caridataobateresepBaru()
    {
        if ($this->request->isAJAX()) {
            ...
            $data = [
                'tampildata' => $gudang->caridataobatbaruinput($lokasi, $code),
    ...
    public function caridataobateresepBaruAKHP()
    {
        if ($this->request->isAJAX()) {
            ...
            $data = [
                'tampildata' => $gudang->caridataobatbaruinput($lokasi, $code, $namaobat),
    ...
    public function printEResep()
    {
        $resume = new ModelTerimaPBFDetail();
        $asesment = new  ModelPelayananPoliRMEMedis();
        $journalnumber = $this->request->getVar('journalnumber');

        $headerpelayanan = $resume->search_header_pelayanan($journalnumber);
        $data = [
            'asesment' => $asesment->select('id, referencenumber, bb, alergi, alergiObat')->where('referencenumber', $headerpelayanan['referencenumber'])->first(),
            'headerpelayanan' => $headerpelayanan,
            'DetailObat' => $resume->search_detail_pelayanan($journalnumber),
        ];
        

        return view('cetakan/print_e_resep', $data);
    }
}
