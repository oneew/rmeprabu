...
class RegBD extends BaseController
{
    ...
    public function order()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = [
                'sukses' => view('bankdarah/modaldaftarBD', $data)
            ];
            echo json_encode($msg);
        }
    }
    ...
    public function orderfromrajal()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = [
                'sukses' => view('bankdarah/modaldaftarBDfromrajal', $data)
            ];
            echo json_encode($msg);
        }
    }
    ...
    public function simpandata()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$valid) {
                ...
                $query = $db->query("SELECT MAX(journalnumber) as kode_jurnal,  MAX(numberseq)as antrian FROM transaksi_pelayanan_penunjang_header WHERE  documentdate='$documentdate' AND groups='BD' LIMIT 1");
                ...
                $msg = [
                    'sukses' => 'Pendaftaran Pelayanan Bank Darah Sudah Berhasil Dilakukan'
                ];
            }
            echo json_encode($msg);
        } else {
    ...
    public function simpandatafromrajal()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$valid) {
                ...
                $newkode = $visited . $underscore . $lokasi . $underscore . $today . $underscore . $token . $underscore . $pasienid . $underscore . sprintf('%06s', $nourut);
                $numberseq = $antrian + 1;
                ...
                $perawat->insert($simpandata);
                $msg = [
                    'sukses' => 'Pendaftaran Bank Darah Sudah Berhasil Dilakukan'
                ];
            }
            echo json_encode($msg);
        } else {
    ...
    public function simpandataNonRM()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$valid) {
                ...
                $query = $db->query("SELECT MAX(journalnumber) as kode_jurnal, MAX(numberseq)as antrian FROM transaksi_pelayanan_penunjang_header WHERE  documentdate='$documentdate' AND groups='BD' AND types='NRM' LIMIT 1");
                foreach ($query->getResult() as $row) {
                    $kode = $row->kode_jurnal;
                    $antrian = $row->antrian;
                }
                ...
                $simpandata = [
                    ...
                    'pasiensubareaname' => $this->request->getVar('pasiensubareaname'),
                    'paymentmethod' => $this->request->getVar('paymentmethod'),
                    'paymentmethodname' => $this->request->getVar('paymentmetho'),
                    'faskes' => $this->request->getVar('faskes'),
                    'faskesname' => $this->request->getVar('faskesname'),
                    'dokter' => $this->request->getVar('dokter'),
                    'doktername' => $this->request->getVar('doktername'),
                ...
                $perawat->insert($simpandata);
                $msg = [
                    'sukses' => 'Pendaftaran Bank Darah Pasien Datang langsung Sudah Berhasil Dilakukan'
                ];
            }
            echo json_encode($msg);
        } else {
