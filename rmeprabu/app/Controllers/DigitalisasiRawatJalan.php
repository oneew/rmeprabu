...
use App\Models\ModelDigitalisasiRM;
use Config\Services;
...
class DigitalisasiRawatJalan extends BaseController
{
    ...
    public function index()
    {
        ...
        return view('digitalisasiRM/registerrajal', $data);
    }
    ...
    public function ambildata()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = [
                'data' => view('digitalisasiRM/dataregisterrajal', $data)
            ];
    ...
    public function Cetak()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = [
                'sukses' => view('digitalisasiRM/modalprintregisterrajal', $data)
            ];
            echo json_encode($msg);
        } else {
            exit('tidak dapat diproses');
    ...
    public function HistoriKunjungan()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = [
                'data' => view('digitalisasiRM/data_histori_kunjungan', $data)
            ];
            echo json_encode($msg);
        } else {
            exit('tidak dapat diproses');
    ...
    public function UbahRajal()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = [
                'sukses' => view('digitalisasiRM/modalubahrajal', $data)
            ];
            echo json_encode($msg);
        } else {
            exit('tidak dapat diproses');
    ...
    public function simpanSEP()
    {
        if ($this->request->isAJAX()) {
            ...
            } else if (($tandakontrol == 1) and ($datasep['noSurat'] == "")) {
                $msg = [
                    'gagal' => true,
                    'pesan' => 'No Surat Kontrol Tidak Boleh Kosong Untuk Kunjungan Spesialis/ SubSpesialis!!!'
                ];
            }
    ...
    public function simpanDataArsipRajal()
    {
        ...
        if ($this->request->isAJAX()) {
            ...
            $validation->setRules([
                'referencenumber' => [
                    'rules' => 'required',
                    'errors' => [
                        'required' => 'belum memilih Pasien',
                    ]
                ],
                'ibsdoktername' => [
                    'rules' => 'required',
                    'errors' => [
                        'required' => 'belum memilih nama Dokter',
                    ]
                ]
            ]);
            ...
            if ($isValid == false) {
                ...
                $name = [];
                $nipTanpaSpasi = preg_replace('/\s+/', '', $this->request->getVar('pasienid'));
                if (isset($files['file-foto'])) {
                    foreach ($files['file-foto'] as $i => $img) {
                        if ($img->isValid() && !$img->hasMoved()) {
                            //$newName = (time() * 1000) . '_' . $nipTanpaSpasi . '.' . $img->guessExtension();
                            $newName = (time() * 1000) . '_' . $nipTanpaSpasi . '_' . $i . '.' . $img->guessExtension();
                            $img->move(FCPATH . 'uploadRM', $newName);
                            $name[$i] = $newName;
                        }
                    }

                    $file_arsip = json_encode($name);
                    $simpandata = [
                        'groups' => $this->request->getVar('groups'),
                        'referencenumber' => $this->request->getVar('referencenumber'),
                        'pasienid' => $this->request->getVar('pasienid'),
                        ...
                        'poliklinikname' => $this->request->getVar('poliklinikname'),
                        'doktername' => $this->request->getVar('ibsdoktername'),
                        'paymentmethodname' => $this->request->getVar('paymentmethodname2'),
                        'fileArsip' => $file_arsip,
                        'createdBy' => $this->request->getVar('createdBy'),

                    ];
                    $datadiri = new ModelDigitalisasiRM();
                    $datadiri->insert($simpandata);
                    $msg = [
                        'sukses' => 'Data Arsip Disimpan',
    ...
    public function ViewDataArsipDetailRajal()
    {
        if ($this->request->isAJAX()) {

            $id = $this->request->getVar('id');

            $pegawai = new ModelDigitalisasiRM;
            $data = [

                'detail_arsip' => $pegawai->get_data_arsip_digital_detail($id),
            ];
            $msg = [
                'sukses' => view('digitalisasiRM/modalviewarsip_detail_rajal', $data)
            ];
            echo json_encode($msg);
        }
    }

    public function Historiupload()
    {
        $data = [
            'list' => $this->data_payment(),
            'pilihanpoli' => $this->smf(),
        ];
        return view('digitalisasiRM/registerrajal_histori_upload', $data);
    }

    public function ambildatahistoriupload()
    {
        if ($this->request->isAJAX()) {

            $register = new ModelDigitalisasiRM();
            $data = [
                'tampildata' => $register->ambildatarajalhistoriupload()
            ];
            $msg = [
                'data' => view('digitalisasiRM/dataregisterrajal_histori_upload', $data)
            ];
            echo json_encode($msg);
        } else {
            exit('tidak dapat diproses');
    ...
    public function caridataregisterHistoriUpload()
    {
        if ($this->request->isAJAX()) {
            ...
            $register = new ModelDigitalisasiRM();
            $data = [
                'tampildata' => $register->search_RegisterRajalhistoriupload($search)
            ];
    ...
    public function hapusArsip()
    {
        if ($this->request->isAJAX()) {
            ...
            $id = $this->request->getVar('id');
            $TNO = new ModelDigitalisasiRM;
            $TNO->delete($id);

            $msg = [
                'sukses' => "Data Arsip dengan ID : $id Berhasil dihapus"
            ];
