...
class StockOpnameGudang extends BaseController
{
    ...
    public function index()
    {
        ...
        echo view('gudangfarmasi/form_stock_opname', $data);
    }
    ...
    public function simpandatastockopname()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$valid) {
                ...
                $query = $db->query("SELECT MAX(journalnumber) as kode_jurnal, MAX(numberseq)as noantrian FROM transaksi_farmasi_opname_gudang_header WHERE  documentyear='$tahun' AND groups='$groups' LIMIT 1");
                ...
                $token_so = strtoupper($token3);

                $newkode = $token_so . $underscore . $groups . $underscore . $pbf . $underscore . $potongtahun . $underscore . sprintf('%07s', $nourut);
                ...
                $perawat = new ModelSOGudangHeader;
    ...
    public function simpandastockopname_detail()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$valid) {
                ...
                $perawat = new ModelSOGudangDetail;
                $perawat->insert($simpandata);
                $msg = [
    ...
    public function resumeDetail()
    {
        if ($this->request->isAJAX()) {
            ...
            $journalnumber = $this->request->getVar('journalnumber');
            $data = [
                'DetailObat' => $resume->search_detail_so_gudang($journalnumber)
            ];
            $msg = [
                'data' => view('gudangfarmasi/data_form_so_gudang', $data)
            ];
            echo json_encode($msg);
        } else {
            exit('tidak dapat diproses');
    ...
    public function DSO()
    {
        ...
        return view('gudangfarmasi/registerDSO', $data);
    }
    ...
    public function ambildataDSO()
    {
        if ($this->request->isAJAX()) {
            ...
            $register = new ModelSOGudangHeader();
            $data = [
                'tampildata' => $register->ambildataDSO()
            ];
            $msg = [
                'data' => view('gudangfarmasi/dataregisterDSO', $data)
            ];
            echo json_encode($msg);
        } else {
            exit('tidak dapat diproses');
    ...
    public function DetailDSO($id = '')
    {
        ...
        return view('gudangfarmasi/form_stock_opname_add', $data);
    }
    ...
    public function Search_BacthNumber()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = [
                'data' => view('gudangfarmasi/modalcaribatchnumber', $data)
            ];
            echo json_encode($msg);
        } else {
            exit('tidak dapat diproses');
    ...
    public function printstockopname()
    {
        ...
        $data = [
            'dataopname' => $pasien->opnameheader($id),
            'header1' => $row2['header1'],
            'header2' => $row2['header2'],
            'status' => $row2['status'],
            'alamat' => $row2['alamat'],
            'deskripsi' => $row2['deskripsi'],
            'tampildata' => $pasien->opnamedetail($id),
        ];

        $html = view('pdf/stylebootstrap');
        $html .= view('pdf/so_gudang', $data);
    ...
    public function KoreksiBatch()
    {
        ...
        echo view('gudangfarmasi/form_stock_opname_koreksi_batch', $data);
    }


    public function simpandastockopname_detail_koreksi_batch()
    {
        if ($this->request->isAJAX()) {
            $validation = \Config\Services::validation();
            $valid = $this->validate([
            ...
            if (!$valid) {
                ...
                $cekbatch = $this->request->getVar('batchbaru');
                ...
                if ($cekbatch == 0) {
                    ...
                    $perawat2 = new ModelSOGudangDetail;
                    $perawat2->insert($simpandatabaru);
                    $msg = [
