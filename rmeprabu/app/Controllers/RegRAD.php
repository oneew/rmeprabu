...
use App\Models\ModelDaftarRadiologiNonRM;
use App\Models\ModelPasien;
use Config\Services;
use Dompdf\Dompdf;


class RegRAD extends BaseController
{
    ...
    public function order()
    {
        if ($this->request->isAJAX()) {
            ...
            $data = [
                ...
                'bed' => $this->bed(),
                'namasmf' => $this->smf(),
                'Radiologi' => $this->_data_radiologi(),

            ];
            $msg = [
                'sukses' => view('radiologi/modaldaftarradiologi', $data)
            ];
            echo json_encode($msg);
        }
    }
    ...
    public function orderfromrajal()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = [
                'sukses' => view('radiologi/modaldaftarradiologifromrajal', $data)
            ];
            echo json_encode($msg);
        }
    }
    ...
    public function simpandata()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$valid) {
                ...
                $tanggalpelayanan = $this->request->getVar('tgl_order');
                ...
                $simpandata = [
                    ...
                    'tgl_order' => $documentdate,
                    'token_radiologi' => $this->request->getPost('token_radiologiranap'),
                ...
                $msg = [
                    'sukses' => 'Pendaftaran Radiologi Sudah Berhasil Dilakukan'
                ];
            }
            echo json_encode($msg);
        } else {
    ...
    public function simpandatafromrajal()
    {
        if ($this->request->isAJAX()) {
            $validation = \Config\Services::validation();
            $valid = $this->validate([
            ...
            if (!$valid) {
                ...
                helper('text');
                $token4 = random_string('alnum', 3);
                $token_reborn3 = strtoupper($token4);
                $newkode = $visited . $underscore . $lokasi . $token_reborn3. $underscore . $today . $underscore . sprintf('%06s', $nourut);
                ...
                $simpandata = [
                    ...
                    'orderpemeriksaan' => $this->request->getPost('orderpemeriksaan'),
                    'tgl_order' => $documentdate,
                    'token_radiologi' => $this->request->getPost('token_radiologirajaligd'),
                    'memo' => $this->request->getPost('memo'),
                    'note' => $this->request->getPost('note'),
                    'status' => $this->request->getPost('status'),
    ...
    private function _data_radiologi()
    {
        $request = Services::request();
        $m_auto = new Model_autocomplete($request);
        $list = $m_auto->get_list_pelayanan_radiologi();
        return $list;
    }
    ...
    public function simpandataNonRM()
    {
        if ($this->request->isAJAX()) {
            $validation = \Config\Services::validation();
            $valid = $this->validate([
            ...
            if (!$valid) {
                ...
                $tglpelayanan = $this->request->getVar('documentdate');
                $splitwaktu = explode("-", $tglpelayanan);
                $documentmonth = $splitwaktu[1];
                $documentyear = $splitwaktu[0];

                $query = $db->query("SELECT MAX(journalnumber) as kode_jurnal, MAX(numberseq)as antrian FROM transaksi_pelayanan_penunjang_header WHERE  documentdate='$documentdate' AND groups='RAD' AND types='NRM' LIMIT 1");
                ...
                $pasiendateofbirth = date('Y-m-d', strtotime($this->request->getVar("pasiendateofbirth")));

                $dob = strtotime($pasiendateofbirth);
                $current_time = time();
                ...
                $perawat = new ModelDaftarRadiologiNonRM;
                $perawat->insert($simpandata);
                $msg = [
                    'sukses' => 'Pendaftaran Radiologi Pasien Datang langsung Sudah Berhasil Dilakukan'
                ];
            }
            echo json_encode($msg);
        } else {
    ...
    public function simpandataNonRM_PasienBaru()
    {
        if ($this->request->isAJAX()) {
            $validation = \Config\Services::validation();
            $valid = $this->validate([
            ...
            if (!$valid) {
                ...
                $tglpelayanan = $this->request->getVar('documentdate_baru');
                $splitwaktu = explode("-", $tglpelayanan);
                $documentmonth = $splitwaktu[1];
                $documentyear = $splitwaktu[0];
                ...
                $pasiendateofbirth = date('Y-m-d', strtotime($this->request->getVar("tanggallahir")));
                ...
                $simpandataRM = [
                        ...
                        'area' => $this->request->getVar('area_baru'),
                        'subarea' => $this->request->getVar('kodewilayah_baru'),
                        'subareaname' => $this->request->getVar('namasubarea_baru'),
                        'address' => $this->request->getVar('alamat_baru'),
                        ...
                        'paymentmethod' => $this->request->getVar('carabayar_baru'),
                        'paymentmethodname' => $this->request->getVar('carabayar_baru'),
                        ...
                        'kecamatan' => $this->request->getVar('namakecamatan_baru'),
                        'kabupaten' => $this->request->getVar('kabupaten_baru'),
                        'propinsi' => $this->request->getVar('propinsi_baru'),
                        'namaibukandung' => $this->request->getVar('namaorangtua'),
                        'incorrectNik' => $incorrectNik,
                        'subdistrict' => $this->request->getVar('namakecamatan_baru'),
                        'provinsi' => $this->request->getVar('propinsi_baru'),
                        'city' => $this->request->getVar('area_baru'),

                    ];
                    $pasien = new ModelPasien;
                    $pasien->insert($simpandataRM);

               

                $simpandata = [
                    'types' => $this->request->getVar('types_baru'),
                    'visited' => $this->request->getVar('visited_baru'),
                    'groups' => $this->request->getVar('groups_baru'),
                    'journalnumber' => $newkode,
                    'documentdate' => $this->request->getVar('documentdate_baru'),
                    'documentyear' => $documentyear,
                    'documentmonth' => $documentmonth,
                    'registernumber' => $this->request->getVar('registernumber_baru'),
                    'referencenumber' => $this->request->getVar('referencenumber_baru'),
                    'registernumber_rawatjalan' => $this->request->getVar('registernumber_rawatjalan_baru'),
                    'registernumber_rawatinap' => $this->request->getVar('registernumber_rawatinap_baru'),
                    'pasienid' => $normbaru,
                    'oldcode' => $this->request->getVar('oldcode'),
                    ...
                    'pasienarea' => $this->request->getVar('area_baru'),
                    'pasiensubarea' => $this->request->getVar('kodewilayah_baru'),
                    'pasiensubareaname' => $this->request->getVar('namasubarea_baru'),
                    'paymentmethod' => $this->request->getVar('carabayar_baru'),
                    'paymentmethodname' => $this->request->getVar('carabayar_baru'),
                    'faskes' => $this->request->getVar('kodefaskes'),
                    'faskesname' => $this->request->getVar('namafaskes'),
                    'dokter' => $this->request->getVar('dokter_baru'),
                    'doktername' => $this->request->getVar('doktername_baru'),
                    'employee' => $this->request->getVar('employee_baru'),
                    'employeename' => $this->request->getVar('employeename_baru'),
                    'smf' => $this->request->getVar('smf_baru'),
                    'smfname' => $this->request->getVar('smfname_baru'),

                    'classroom' => $this->request->getVar('classroom_baru'),
                    'classroomname' => $this->request->getVar('classroomname_baru'),
                    'room' => $this->request->getVar('room_baru'),
                    'roomname' => $this->request->getVar('roomname_baru'),
                    ...
                    'orderpemeriksaan' => $this->request->getPost('orderpemeriksaan'),
                    'tgl_order' => $this->request->getPost('tgl_order_barun'),
                    'token_radiologi' => $this->request->getPost('token_radiologi_baru'),
                    'memo' => $this->request->getPost('memo'),
                    'note' => $this->request->getPost('note'),
                    //'status' => $this->request->getPost('status'),
                    'numberseq' => $numberseq,
