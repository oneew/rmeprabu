...
class RegisterBD extends BaseController
{
    ...
    public function index()
    {
        $data = [
            ...
            'dokterradiologi' => $this->_data_dokter_BD1(),
            'kelas' => $this->_data_kelas(),
            'list' => $this->data_payment(),
            'pilihanpoli' => $this->smf(),
        ];

        return view('bankdarah/datapasien_to_register_bd', $data);
    }
    ...
    private function _data_dokter_BD1()
    {
        $request = Services::request();
        $m_auto = new Model_autocomplete($request);
        $list = $m_auto->get_list_dokter_bd();
        return $list;
    }
    ...
    public function ambildata()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = [
                'data' => view('bankdarah/data_pasien_ranap_to_order_bd', $data)
            ];
    ...
    public function ambildatapasienrajal()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = [
                'data' => view('bankdarah/data_pasien_rajal_to_order_bd', $data)
            ];
            echo json_encode($msg);
        } else {
            exit('tidak dapat diproses');
    ...
    public function caripasiennonRM()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = [
                'data' => view('bankdarah/modalcaripasiennonrm', $data)
            ];
            echo json_encode($msg);
        } else {
            exit('tidak dapat diproses');
    ...
    public function ambildatapasienlama()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = [
                'data' => view('bankdarah/masterdatapasien', $data)
            ];
            echo json_encode($msg);
        } else {
            exit('tidak dapat diproses');
    ...
    public function ambildataBD()
    {
        if ($this->request->isAJAX()) {
            ...
            $data = [
                'tampildata' => $register->ambildatarajalIGD()
            ];
            $msg = [
                'data' => view('bankdarah/data_pasien_rajal_to_order_bd', $data)
            ];
    ...
    public function caridataregisterpoliBD()
    {
        if ($this->request->isAJAX()) {
            ...
            $register = new ModelRawatJalanDaftar();
            $data = [
                'tampildata' => $register->search_RegisterRajaBDl($search)
            ];
    ...
    private function _data_dokter_bd()
    {
        $request = Services::request();
        $m_auto = new Model_autocomplete($request);
        $list = $m_auto->get_list_dokter_bd();
