...
class RekMedReKCBLPK extends BaseController
{
    ...
    public function index()
    {
        ...
        return view('rekammedik/registerlpk_rekapcabar', $data);
    }
    ...
    public function ambildata()
    {
        if ($this->request->isAJAX()) {
            ...
            $row = $m_icd->get_data_lpk_cabar_kop($lokasikasir);
            $register = new ModelPelayananPoli();
            $data = [
                'tampildata' => $register->ambildatalpk_rekapcabar(),
                'header1' => $row['header1'],
                'header2' => $row['header2'],
                'status' => $row['status'],
                'alamat' => $row['alamat'],
            ...
            $msg = [
                'data' => view('rekammedik/datarekapkunjunganlpk_cabar', $data)
            ];
    ...
    public function caridataregisterpoli()
    {
        if ($this->request->isAJAX()) {
            ...
            $register = new ModelPelayananPoli();
            $data = [
                'tampildata' => $register->search_RegisterLPK_rekapcabar($search),
                'header1' => $row['header1'],
                'header2' => $row['header2'],
                'status' => $row['status'],
                'alamat' => $row['alamat'],
