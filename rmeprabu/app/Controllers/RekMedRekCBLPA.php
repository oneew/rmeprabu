...
class RekMedReKCBLPA extends BaseController
{
    ...
    public function index()
    {
        ...
        return view('rekammedik/registerlpa_rekapcabar', $data);
    }
    ...
    public function ambildata()
    {
        if ($this->request->isAJAX()) {
            ...
            $row = $m_icd->get_data_lpa_cabar_kop($lokasikasir);
            $register = new ModelPelayananPoli();
            $data = [
                'tampildata' => $register->ambildatalpa_rekapcabar(),
                'header1' => $row['header1'],
                'header2' => $row['header2'],
                'status' => $row['status'],
                'alamat' => $row['alamat'],
            ...
            $msg = [
                'data' => view('rekammedik/datarekapkunjunganlpA_cabar', $data)
            ];
    ...
    public function caridataregisterpoli()
    {
        if ($this->request->isAJAX()) {
            ...
            $register = new ModelPelayananPoli();
            $data = [
                'tampildata' => $register->search_RegisterLPA_rekapcabar($search),
                'header1' => $row['header1'],
                'header2' => $row['header2'],
                'status' => $row['status'],
                'alamat' => $row['alamat'],
