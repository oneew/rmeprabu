...
class RegFRS extends BaseController
{
    ...
    public function order()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = [
                'sukses' => view('forensik/modaldaftarFRS', $data)
            ];
            echo json_encode($msg);
        }
    }
    ...
    public function orderfromrajal()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = [
                'sukses' => view('forensik/modaldaftarFRSfromrajal', $data)
            ];
            echo json_encode($msg);
        }
    }
    ...
    public function simpandata()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$valid) {
                ...
                $query = $db->query("SELECT MAX(journalnumber) as kode_jurnal FROM transaksi_pelayanan_penunjang_header WHERE  documentdate='$documentdate' AND groups='FRS' LIMIT 1");
                ...
                $simpandata_frs = [
                    ...
                    'request_from' => $this->request->getVar('request_from'),
                    ...
                    'createdby' => $this->request->getVar('createdby'),
                    'token_forensik' => $this->request->getPost('token_radiologi'),
                ];
                $forensik = new ModelDaftarForensik;
                $forensik->insert($simpandata_frs);

                $msg = [
                    'sukses' => 'Pendaftaran Pelayanan Forensik Sudah Berhasil Dilakukan'
                ];
            }
            echo json_encode($msg);
        } else {
    ...
    public function simpandataNonRM()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$valid) {
                ...
                $query = $db->query("SELECT MAX(journalnumber) as kode_jurnal, MAX(numberseq)as antrian FROM transaksi_pelayanan_penunjang_header WHERE  documentdate='$documentdate' AND groups='FRS' AND types='NRM' LIMIT 1");
                foreach ($query->getResult() as $row) {
                    $kode = $row->kode_jurnal;
                    $antrian = $row->antrian;
                }
