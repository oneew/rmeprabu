...
class RegLPA extends BaseController
{
    ...
    public function order()
    {
        if ($this->request->isAJAX()) {
            ...
            $data = [
                ...
                'dokterradiologi' => $this->_data_dokter_patologianatomi(),

            ];
            $msg = [
                'sukses' => view('patologianatomi/modaldaftarpa', $data)
            ];
            echo json_encode($msg);
        }
    }
    ...
    public function orderfromrajal()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = [
                'sukses' => view('patologianatomi/modaldaftarLPAfromrajal', $data)
            ];
            echo json_encode($msg);
        }
    }
    ...
    public function simpandata()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$valid) {
                ...
                $query = $db->query("SELECT MAX(journalnumber) as kode_jurnal, MAX(numberseq)as antrian FROM transaksi_pelayanan_penunjang_header WHERE  documentdate='$documentdate' AND groups='LPA' ORDER BY id DESC LIMIT 1");
                ...
                $msg = [
                    'sukses' => 'Pendaftaran Patologi Anatomi Sudah Berhasil Dilakukan'
                ];
            }
            echo json_encode($msg);
        } else {
    ...
    public function simpandatafromrajal()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$valid) {
                ...
                $query = $db->query("SELECT MAX(journalnumber) as kode_jurnal, MAX(numberseq)as antrian FROM transaksi_pelayanan_penunjang_header WHERE  documentdate='$documentdate' AND groups='LPA' LIMIT 1");
                ...
                $perawat->insert($simpandata);
                $msg = [
                    'sukses' => 'Pendaftaran Laboratorium Patologi Anatomi Sudah Berhasil Dilakukan'
                ];
            }
            echo json_encode($msg);
        } else {
    ...
    public function simpandataNonRM()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$valid) {
                ...
                $usia_pasien = $this->request->getVar('usia');

                if ($usia_pasien == "-") {

                    $tl = date('Y-m-d', strtotime($this->request->getVar("pasiendateofbirth")));
                } else {
                    $usiapasien = $usia_pasien;
                    $thnlahir = date('Y') - $usiapasien;
                    $tl = $thnlahir . '-' . date('m') . '-' . date('d');
                }
                ...
                $query = $db->query("SELECT MAX(journalnumber) as kode_jurnal, MAX(numberseq)as antrian FROM transaksi_pelayanan_penunjang_header WHERE  groups='LPA' AND types='NRM' and memo=1 ORDER BY id DESC LIMIT 1");
                ...
                $perawat->insert($simpandata);
                $msg = [
                    'sukses' => 'Pendaftaran Patologi Anatomi Pasien Datang langsung Sudah Berhasil Dilakukan'
                ];
            }
            echo json_encode($msg);
        } else {
    ...
    private function _data_dokter_patologianatomi()
    {
        $request = Services::request();
        $m_auto = new Model_autocomplete($request);
