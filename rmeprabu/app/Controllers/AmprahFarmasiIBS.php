...
class AmprahFarmasiIBS extends BaseController
{
    ...
    public function simpandataamprah()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$valid) {
                ...
                $newkode = $tanda . $underscore . $locationcode . $underscore . $groups . $underscore . $token_reborn3 . $underscore . $potongtahun . $underscore . sprintf('%07s', $nourut);
    ...
    public function resumeDetail()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = [
                'data' => view('ibs/data_form_amprah', $data)
            ];
            echo json_encode($msg);
        } else {
            exit('tidak dapat diproses');
