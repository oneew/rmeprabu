...
class PelayananRHM extends BaseController
{
    ...
    public function inputdetailRHM2($id = '')
    {
        $id = $this->request->getVar('id');
        ...
        echo view('rehabmedik/detail_rhm', $data);
    }



    public function inputdetailRHM($page = '')
    {
    ...
    public function resumeGabung()
    {
        if ($this->request->isAJAX()) {
            ...
            $journalnumber = $this->request->getVar('journalnumber');
            $data = [
                'Radiologi' => $resume->searchbyjournalnumberRHM($journalnumber)
            ];
            $msg = [
                'data' => view('rehabmedik/data_resume_rhm', $data)
            ];
            echo json_encode($msg);
        } else {
            exit('tidak dapat diproses');
    ...
    public function historiRHM()
    {
        if ($this->request->isAJAX()) {
            ...
            $relation = $this->request->getVar('relation');
            $data = [
                'Radiologi' => $resume->search_histori_RHM($relation)
            ];
            $msg = [
                'data' => view('rehabmedik/data_histori_rhm', $data)
            ];
            echo json_encode($msg);
        } else {
            exit('tidak dapat diproses');
    ...
    public function AddPemeriksaanPaket()
    {
        if ($this->request->isAJAX()) {
            ...
            $data = [
                ...
                'perawatRHM' => $this->perawatRHM(),

            ];
            $msg = [
                'sukses' => view('rehabmedik/modalinputPaketRHM', $data)
            ];
            echo json_encode($msg);
        }
    }
    ...
    public function caripaket()
    {
        if ($this->request->isAJAX()) {
            ...
            $perawatRHM = $this->request->getVar('perawatRHM');
            ...
            $data = [
                ...
                'usia' => $umur,
                'jns_rawat' => '1',
                'namaperawatRHM' => $perawatRHM,

            ];

            $msg = [
                'data' => view('rehabmedik/datapaketRHM', $data)
            ];
            echo json_encode($msg);
        } else {
            exit('tidak dapat diproses');
            ...
            if ($tandai <> null) {
                for ($i = 0; $i < count($tandai); $i++) {
                    ...
                    $new_data[$i] = [
                        ...
                        'jns_rawat' => $data[46],
                        'created_at' => $data[38],
                        'paramedicName' => $data[47],
                    ];
                }
                $radiologi = new ModelPenunjangDetail;
                $radiologi->insertBatch($new_data);
                $msg = [
                    'sukses' => "Pemeriksaan Rehab Medik Sudah ditambahkan"
                ];
            } else {
                $msg = [
    ...
    private function perawatRHM()
    {
        $request = Services::request();
        $m_auto = new Model_autocomplete($request);
        $list = $m_auto->get_list_perawat_rhm();
        return $list;
    }
}
