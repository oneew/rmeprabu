...
class FarmasiPelayananIGD extends BaseController
{
    ...
    public function index()
    {
        ...
        echo view('depofarmasi/form_farmasi_pelayanan_igd', $data);
    }
    ...
    public function simpandataresep()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$valid) {
                ...
                $newkode = $tanda . $underscore . $pasienid . $underscore . $locationcode . $underscore . $token_reborn . $underscore . $today . $underscore . sprintf('%06s', $nourut);
    ...
    public function Search_PasienRanap()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = [
                'data' => view('depofarmasi/modalcaripasienigd', $data)
            ];
            echo json_encode($msg);
        } else {
            exit('tidak dapat diproses');
    ...
    public function caridatapasienranap()
    {
        if ($this->request->isAJAX()) {
            ...
            $data = [
                'tampildata' => $register->Search_Pasien_Igd($search)
            ];


            $msg = [
                'data' => view('depofarmasi/data_pasien_igd', $data)
            ];
            echo json_encode($msg);
        } else {
            exit('tidak dapat diproses');
    ...
    public function resumePelayanan()
    {
        if ($this->request->isAJAX()) {
            ...
            $data = [
                ...
                'carapakai' => $resume->cara_pakai(),
                'carapetunjuk' => $resume->cara_petunjuk(),
                'id_header' => $headerpelayanan['id'],
                'status' => $headerpelayanan['status'],
            ];
            $msg = [
                'data' => view('depofarmasi/data_form_pelayanan_igd', $data)
            ];
            echo json_encode($msg);
        } else {
            exit('tidak dapat diproses');
    ...
    public function DFPR()
    {
        ...
        return view('depofarmasi/registerDepoIGD', $data);
    }
    ...
    public function ambildataDFPR()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = [
                'data' => view('depofarmasi/dataregisterDFIGD', $data)
            ];
            echo json_encode($msg);
        } else {
            exit('tidak dapat diproses');
    ...
    public function caridataDFPR()
    {
        if ($this->request->isAJAX()) {
            ...
            $register = new ModelDepoSPHeader();
            $data = [
                'tampildata' => $register->search_RegisterDFIGD($search)
            ];
    ...
    public function DetailDFPR($id = '')
    {
        ...
        return view('depofarmasi/form_farmasi_pelayanan_igd_add', $data);
    }
    ...
    public function printpenjualan()
    {
        ...
        $id = $this->request->getVar('page');
        $pasien = new ModelPasienRanap($this->request);
        $asal = $pasien->get_asal_header($id);
        $journalnumber = $asal['referencenumber'];

        $asal_triase = $pasien->get_triase($journalnumber);
        $triase = $asal_triase['kelompok_triase'];
        $data = [
        ...
        $html = view('pdf/stylebootstrap');
        $html = view('pdf/struk_depo_igd', $data);
        $dompdf->loadhtml($html);
        $dompdf->setPaper('A4', 'portrait');
        $dompdf->render();
        $namafile = 'Struk Depo IGD';
        $dompdf->stream($namafile, ['Attachment' => 0]);
    }
    ...
    public function printpenjualanKonvesional()
    {
        ...
        return view('cetakan/buktiresepigd', $data);
    }
    ...
    public function printpenjualanKonvesionalKronis()
    {
        ...
        return view('cetakan/buktiresepigdkronis', $data);
    }
    ...
    public function printpenjualanKonvesionalNonKronis()
    {
        ...
        return view('cetakan/buktiresepigdnonkronis', $data);
    }
