...
class FarmasiPelayananAKHPRanap extends BaseController
{
    ...
    public function orderesepranap()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = [
                'sukses' => view('rawatinap/modaleresep_ranap_akhp', $data)
            ];
            echo json_encode($msg);
        }
    }
    ...
    public function resumePelayananAKHPRanap()
    {
        if ($this->request->isAJAX()) {

            $resume = new ModelRanapAKHP();
            $journalnumber = $this->request->getVar('journalnumber');
            $headerpelayanan = $resume->search_header_pelayanan_AKHP($journalnumber);

            $nomorjurnal = $headerpelayanan['referencenumber'];
