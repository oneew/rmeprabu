...
use App\Models\ModelrajalSO;
use App\Models\ModelSODepoHeaderRuangan;
use CodeIgniter\HTTP\Request;
use Config\Services;
use Dompdf\Dompdf;
use App\Models\ModelSODepoDetailCL;
use App\Models\ModelSODepoHeaderCL;



class StockOpnameIBS extends BaseController
{
    ...
    public function index()
    {
        ...
        $data = [
            ...
            'lokasi' => $this->lokasiruanganSOIBS(),
        ];

        echo view('ibs/form_stock_opname', $data);
    }
    ...
    public function simpandatastockopname()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$valid) {
                ...
                $token_reborn = strtoupper($token3);

                $newkode = $tanda . $underscore . $groups . $underscore . $token_reborn . $underscore . $potongtahun . $underscore . sprintf('%07s', $nourut);
                ...
                $perawat = new ModelSODepoHeaderCL;
    ...
    public function simpandastockopname_detail()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$valid) {
                ...
                $perawat = new ModelSODepoDetailCL;
                $perawat->insert($simpandata);
                $msg = [
    ...
    public function DSO()
    {
        ...
        return view('ibs/registerDSO', $data);
    }
    ...
    public function ambildataDSO()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = [
                'data' => view('ibs/dataregisterDSO', $data)
            ];
            echo json_encode($msg);
        } else {
            exit('tidak dapat diproses');
    ...
    public function lokasiruanganSO()
    {

        $m_auto = new ModelrajalSO();
        $list = $m_auto->get_list_lokasi_farmasi();
        return $list;
    }


    public function ambildataDSORuangan()
    {
        if ($this->request->isAJAX()) {

            $register = new ModelSODepoHeaderRuangan();
            $data = [
                'tampildata' => $register->ambildataDSO()
            ];
            $msg = [
    ...
    public function caridataDSORuangan()
    {
        if ($this->request->isAJAX()) {
    ...
    public function lokasiruanganSOIBS()
    {

        $m_auto = new ModelrajalSO();
        $list = $m_auto->get_list_lokasi_farmasi_ibs();
        return $list;
    }
}
