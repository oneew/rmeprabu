...
class RawatJalan extends BaseController
{
    ...
    public function simpandataregister()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$valid) {
                ...
                $newkode = $groups . $underscore . $pasienid . $lokasi . $underscore  . $today . $underscore . sprintf('%06s', $nourut);
                if ($antrian == "") {
                    $nourutantrian = '1';
                } else {
    ...
    public function UbahRajal2()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = [
                'sukses' => view('rawatjalan/modalubahrajalranap', $data)
            ];
            echo json_encode($msg);
        } else {
            exit('tidak dapat diproses');
