...
class RegHD extends BaseController
{
    ...
    public function order()
    {
        if ($this->request->isAJAX()) {
            ...
            $data = [
                ...
                'listDokterHD' => $this->_data_dokter_HD(),

            ];
            $msg = [
                'sukses' => view('hemodialisa/modaldaftarhemodialisa', $data)
            ];
            echo json_encode($msg);
        }
    }
    ...
    public function orderfromrajal()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = [
                'sukseshd' => view('hemodialisa/modaldaftarhdfromrajal', $data)
            ];
            echo json_encode($msg);
        }
    }
    ...
    public function simpandata()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$valid) {
                ...
                $query = $db->query("SELECT MAX(journalnumber) as kode_jurnal FROM transaksi_pelayanan_penunjang_header WHERE  documentdate='$documentdate' AND groups='HD' LIMIT 1");
                ...
                $msg = [
                    'sukses' => 'Pendaftaran Hemodialsia Sudah Berhasil Dilakukan'
                ];
            }
            echo json_encode($msg);
        } else {
    ...
    public function simpandatafromrajal()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$valid) {
                ...
                $perawat->insert($simpandata);
                $msg = [
                    'sukses' => 'Pendaftaran Hemodialisa Sudah Berhasil Dilakukan'
                ];
            }
            echo json_encode($msg);
        } else {
    ...
    public function simpandataNonRM()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$valid) {
                ...
                $query = $db->query("SELECT MAX(journalnumber) as kode_jurnal, MAX(numberseq)as antrian FROM transaksi_pelayanan_penunjang_header WHERE  documentdate='$documentdate' AND groups='HD' AND types='NRM' LIMIT 1");
                ...
                $perawat->insert($simpandata);
                $msg = [
                    'sukses' => 'Pendaftaran Hemodialisa Pasien Datang langsung Sudah Berhasil Dilakukan'
                ];
            }
            echo json_encode($msg);
        } else {
    ...
    private function _data_dokter_HD()
    {
        $request = Services::request();
        $m_auto = new Model_autocomplete($request);
        $list = $m_auto->get_list_dokter_HD();
        return $list;
    }
}
