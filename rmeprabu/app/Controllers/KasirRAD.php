...
class KasirRAD extends BaseController
{
    ...
    public function Validasi()
    {
        ...
        return view('kasirPNJ/registerkasirpenunjang', $data);
    }
    ...
    public function ambildataBelumValidasi()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = [
                'data' => view('kasirPNJ/dataregisterkasirpenunjang', $data)
            ];
    ...
    public function lihatrincianpenunjang()
    {
        if ($this->request->isAJAX()) {
            $id = $this->request->getVar('id');
            ...
            $msg = [
                'suksespenunjang' => view('kasirPNJ/modalpembayaranpenunjang', $data)
            ];
            echo json_encode($msg);
        } else {
            exit('tidak dapat diproses');
    ...
    public function resumeGabung()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = [
                'data' => view('kasirPNJ/data_resume_gabung_kasirPNJ', $data)
            ];
            echo json_encode($msg);
        } else {
            exit('tidak dapat diproses');
    ...
    public function validasipembayaranPNJ()
    {
        if ($this->request->isAJAX()) {
            $validation = \Config\Services::validation();
            $valid = $this->validate([
            ...
            if (!$valid) {
                ...
                $kwi = "KWI";
                $documentdate = date('Y-m-d');
                $query = $db->query("SELECT MAX(journalnumber) as kode_jurnal FROM transaksi_kasir_penunjang WHERE  created_at='$documentdate' ORDER BY id DESC LIMIT 1");
                foreach ($query->getResult() as $row) {
                    $kode = $row->kode_jurnal;
                }
                ...
                helper('text');
                $tokenpenunjang = random_string('alnum', 6);

                $newkode = $kwi . '_' . $kodekasir . '_'. $tokenpenunjang . '_' . $today . '_' . sprintf('%06s', $nourut);
                ...
                if ($pay < 0) {
                    ...
                    $perawat->insert($simpandata);
                    $msg = [
                        'sukses' => 'Validasi pembayaran Penunjang Berhasil',
    ...
    public function AfterValidasi()
    {
        ...
        return view('kasirPNJ/registerkasirpenunjangvalidasi', $data);
    }
    ...
    public function ambildataAfterValidasi()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = [
                'data' => view('kasirPNJ/dataregisterkasirpenunjang_aftervalidasi', $data)
            ];
            echo json_encode($msg);
        } else {
            exit('tidak dapat diproses');
    ...
    public function lihatrincianpenunjang_validasi()
    {
        if ($this->request->isAJAX()) {
            ...
            $m_icd = new ModelPasienMaster();
            $data = [
                'pasienlama' => $m_icd->get_data_penunjang_validasi($id),
                'pasienstatus' => $this->status_pasien(),
            ];
            $msg = [
                'suksespenunjang' => view('kasirPNJ/modalpembayaranpenunjang_validasi', $data)
            ];
            echo json_encode($msg);
        } else {
            exit('tidak dapat diproses');
    ...
    public function update_validasipembayaranPNJ()
    {
        if ($this->request->isAJAX()) {
            $validation = \Config\Services::validation();
            $valid = $this->validate([
    ...
    public function BatalValidasi()
    {
        if ($this->request->isAJAX()) {
            ...
            $kasir = new ModelValidasiPembayaranPenunjang();
            $databayar = $kasir->ambildatapembayaran($journalnumber);
            $id = $databayar['id'];
    ...
    public function BeritaAcara()
    {
        ...
        return view('kasirPNJ/registerkasirpenunjangberitaacara', $data);
    }


    public function ambildataberitaacaraPNJ()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = [
                'data' => view('kasirPNJ/dataregister_penunjang_beritaacara', $data)
            ];
            echo json_encode($msg);
        } else {
            exit('tidak dapat diproses');
    ...
    public function caridataberitaacaraPNJ()
    {
        if ($this->request->isAJAX()) {
            $lokasikasir = "KASIR PENUNJANG MEDIK";
            $m_icd = new ModelPasienRanap($this->request);
    ...
    public function printdetailkwitansiTagihanKonvensional()
    {
        ...
        $resume = new ModelTNODetailRJ();
        $referencenumber = $this->request->getVar('page');
        $nama_print = session()->get('firstname');
        $data = [
            'datapasien' => $pasien->kunjunganpenunjangprintTagihanPenunjang($journalnumber),
            'header1' => $row2['header1'],
            'header2' => $row2['header2'],
        ...
        // $html = view('pdf/stylebootstrap');
        // $html = view('cetakan/printdetailtindakanpenunjang', $data);
        // $dompdf->loadhtml($html);
        // $dompdf->setPaper('A4', 'portrait');
        // $dompdf->render();
        // $namafile = 'Struk radiologi';
        // $dompdf->stream($namafile, ['Attachment' => 0]);
        return view('cetakan/printdetailtindakanpenunjang', $data);
    }

    public function printdetailkwitansiTagihanKonvensional2()
    {
        $dompdf = new Dompdf();
        ...
        $nama_print = session()->get('firstname');
        $data = [
            'datapasien' => $pasien->kunjunganpenunjangprintTagihanPenunjang2($journalnumber),
            'header1' => $row2['header1'],
            'header2' => $row2['header2'],
            'status' => $row2['status'],
            'alamat' => $row2['alamat'],
        ...
        return view('cetakan/printdetailtindakanpenunjangrad', $data);
    }
    ...
    public function printdetailkwitansiTagihanKonvensionalKasir()
    {
        ...
        $resume = new ModelTNODetailRJ();
        $referencenumber = $this->request->getVar('page');
        $nama_print = session()->get('firstaname');
        $data = [
            'datapasien' => $pasien->kunjunganpenunjangprint($journalnumber),
            'header1' => $row2['header1'],
            'header2' => $row2['header2'],
