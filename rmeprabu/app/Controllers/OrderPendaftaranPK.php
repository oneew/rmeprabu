...
class OrderPendaftaranPK extends BaseController
{
    ...
    public function order()
    {
        if ($this->request->isAJAX()) {
            ...
            $data = [
                ...
                'bed' => $this->bed(),
                'namasmf' => $this->smf(),
                'Radiologi' => $this->_data_pk(),
                'kelompokLab' => $this->kelompokLab(),

            ];
            $msg = [
                'sukses' => view('rawatinap/modalorderdaftarpk', $data)
            ];
            echo json_encode($msg);
        }
    }
    ...
    public function simpandata()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$valid) {
                ...
                $msg = [
                    'sukses' => 'Order Pemeriksaan Laboratorium Patologi Klinik Sudah Dikirimkan',
                    'JN' => $newkode,
                    'classroom' => $classroom
                ];
            }
    ...
    private function _data_pk()
    {
        $request = Services::request();
        $m_auto = new Model_autocomplete($request);
        $list = $m_auto->get_list_pelayanan_PK();
        return $list;
    }
    ...
    public function orderpkrajal()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = [
                'sukses' => view('rawatjalan/modalorderdaftarpk_rajal', $data)
            ];
            echo json_encode($msg);
        }
    }

    public function simpandataorderrajal()
    {
        if ($this->request->isAJAX()) {
            $validation = \Config\Services::validation();
            $valid = $this->validate([
            ...
            if (!$valid) {
                ...
                $simpandata = [
                    ...
                    'paymentmethodcardnumber' => $this->request->getVar('paymentmethodcardnumber'),
                    'faskes' => $this->request->getVar('faskes'),
                    'faskesname' => $this->request->getVar('faskesname'),
                    'dokter' => $this->request->getVar('ibsdokter'),
                    'doktername' => $this->request->getVar('ibsdoktername'),
    ...
    public function orderpkigd()
    {
        if ($this->request->isAJAX()) {
            ...
            $data = [
                ...
                'Radiologi' => $this->_data_pk(),
                'kelompokLab' => $this->kelompokLab(),
                'dokterradiologi' => $this->_data_dokter_pk(),

            ];
            $msg = [
                'sukses' => view('igd/modalorderdaftarpk_igd', $data)
            ];
            echo json_encode($msg);
        }
    }
    ...
    public function simpandataorderigd()
    {
        if ($this->request->isAJAX()) {
            $validation = \Config\Services::validation();
            $valid = $this->validate([
            ...
            if (!$valid) {
                ...
                $newkode = $visited . $underscore . $lokasi . $underscore . $token_reborn . $underscore . $today . $underscore . sprintf('%06s', $nourut);
                $orderpemeriksaan = implode(",", $this->request->getPost('orderpemeriksaan'));
    ...
    private function _data_dokter_pk()
    {
        $request = Services::request();
        $m_auto = new Model_autocomplete($request);
