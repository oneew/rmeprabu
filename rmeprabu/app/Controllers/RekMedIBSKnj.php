...
class RekMedIBSKnj extends BaseController
{
    ...
    public function index()
    {
        ...
        return view('rekammedik/registeribs_rekapwilayah', $data);
    }
    ...
    public function ambildata()
    {
        if ($this->request->isAJAX()) {
            ...
            $row = $m_icd->get_data_ibs_wilayah_kop($lokasikasir);
            $register = new ModelPelayananPoli();
            $data = [
                'tampildata' => $register->ambildataibs_rekapwilayah(),
                'header1' => $row['header1'],
                'header2' => $row['header2'],
                'status' => $row['status'],
                'alamat' => $row['alamat'],
            ...
            $msg = [
                'data' => view('rekammedik/datarekapkunjunganibs_wilayah', $data)
            ];
    ...
    public function caridataregisterpoli()
    {
        if ($this->request->isAJAX()) {
            ...
            $register = new ModelPelayananPoli();
            $data = [
                'tampildata' => $register->search_RegisterIBS_rekapwilayah($search),
                'header1' => $row['header1'],
                'header2' => $row['header2'],
                'status' => $row['status'],
                'alamat' => $row['alamat'],
