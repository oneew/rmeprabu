...
class RegABL extends BaseController
{
    ...
    public function order()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = [
                'sukses' => view('ambulance/modaldaftarABL', $data)
            ];
            echo json_encode($msg);
        }
    }
    ...
    public function orderfromrajal()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = [
                'sukses' => view('ambulance/modaldaftarABLfromrajal', $data)
            ];
            echo json_encode($msg);
        }
    }
    ...
    public function simpandata()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$valid) {
                ...
                $query = $db->query("SELECT MAX(journalnumber) as kode_jurnal FROM transaksi_pelayanan_penunjang_header WHERE  documentdate='$documentdate' AND groups='ABL' LIMIT 1");
                ...
                $simpandata_ambulance = [
                    ...
                    'createdby' => $this->request->getVar('createdby'),
                    'token_ambulance' => $this->request->getPost('token_radiologi'),
                ];
                $ambulance = new ModelDaftarAmbulance;
                $ambulance->insert($simpandata_ambulance);

                $msg = [
                    'sukses' => 'Pendaftaran Pelayanan Ambulance Sudah Berhasil Dilakukan'
                ];
            }
            echo json_encode($msg);
        } else {
    ...
    public function simpandataNonRM()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$valid) {
                ...
                $query = $db->query("SELECT MAX(journalnumber) as kode_jurnal, MAX(numberseq)as antrian FROM transaksi_pelayanan_penunjang_header WHERE  documentdate='$documentdate' AND groups='ABL' AND types='NRM' LIMIT 1");
                foreach ($query->getResult() as $row) {
                    $kode = $row->kode_jurnal;
                    $antrian = $row->antrian;
                }
                ...
                $simpandata_ambulance = [
                    ...
                    'bednumber' => $bednumber,
                    'createdby' => $this->request->getVar('createdby'),
                    'token_ambulance' => $this->request->getPost('token_lpa'),
                ];
                $ambulance = new ModelDaftarAmbulance;
                $ambulance->insert($simpandata_ambulance);
                $msg = [
                    'sukses' => 'Pendaftaran Ambulance Pasien Luar Sudah Berhasil Dilakukan'
                ];
            }
            echo json_encode($msg);
        } else {
