...
class Model_autocomplete extends Model
{
    ...
    function get_list_dokter()
    {
        ...
        $this->dt->select('code, name , id , kode_bpjs');
    ...
    function get_list_dokter_all()
    {
        $this->dt = $this->db->table('dokter');
        $this->dt->orderBy('name');
        $this->dt->select('id, code, name, kode_bpjs');
        $this->dt->where('types', 'DOKTER');
        $this->dt->where('inactive', 'TIDAK');
    ...
    function get_list_pelayanan_Gizi($term, $key)
    {
        ...
        $this->dt->where('classroom', $term);
        $this->dt->like('name', $key);
        $this->dt->like('categoryname', 'ASUHAN PELAYANAN GIZI');
        $this->dt->limit(10);
        $query = $this->dt->get();
        return $query->getResultArray();
    }
    ...
    function get_list_wilayah($key)
    {
        ...
        $this->dt->orderBy('master_desakel.ID');
    ...
    {
        $this->dt = $this->db->table('dokter');
        $this->dt->like('memo', 'GIZI');
        $this->dt->like('types', 'NONDOKTER');
        $this->dt->like('inactive', 'TIDAK');
        $this->dt->like('types', 'NONDOKTER');
    ...
    function get_list_pemeriksaan_LPA_paket($kelompokLab, $classroom)
    {
        ...
        $this->dt = $this->db->table('pelayanan_tarif_penunjang');
        $this->dt->orderBy('name');
        $this->dt->select('id, name, price, code, groups, groupname, category, categoryname, expertisegroup, bhp, share1, share2, kelompokLab, bhp');
        $this->dt->like('kelompokLab', $kelompokLab);
        $this->dt->where('classroom', $classroom);
    ...
    {
        $this->dt = $this->db->table('daftar_perawat');
        $this->dt->where('locationname', 'RHM');
        $this->dt->orderBy('nama');
        $query = $this->dt->get();
        return $query->getResultArray();
    }
    ...
    function get_data_lokasi_user()
    {
        $this->dt = $this->db->table('gudang');
        $this->dt->select(' name , id , code ');
        $this->dt->where('code', $this->request->getPost('key'));
        $query = $this->dt->get();
        return $query->getRowArray();
    }

    function get_list_apoteker()
    {
        $this->dt = $this->db->table('petugas_resep');
        $this->dt->orderBy('id');
        $this->dt->select(' name , id, code ');
    ...
    function get_cek_ranap($nomor_id)
    {
        $this->dt = $this->db->table('transaksi_pelayanan_daftar_rawatinap');
        $this->dt->where('id', $nomor_id);
        $this->dt->limit(1);
        $query = $this->dt->get();
        return $query->getRowArray();
    }

    function get_list_room_ok()
    {
        $this->dt = $this->db->table('rooms');
        $this->dt->orderBy('room');
        $this->dt->select(' room , kode ');
        $query = $this->dt->get();
        return $query->getResultArray();
    }
    ...
    function get_data_ats()
    {
        ...
        $this->dt->select(' name , id , nilai');
        $this->dt->where('id', $this->request->getPost('key'));
        $query = $this->dt->get();
        return $query->getRowArray();
    }

    function get_list_aturan_pakai($key)
    {
