...
class ModelTerimaPBFDetail extends Model
{
    ...
    protected $allowedFields = [
        ...
        'createddate', 'pabrik', 'hibah', 'konsinyasi'
    ];
    ...
    function update_validasitelaah($Telaahresep, $simpandata)
    {
        $this->dt = $this->db->table('transaksi_farmasi_pelayanan_header');
        $this->dt->where('Telaahresep', $Telaahresep);
        $this->dt->update($simpandata);
    }
    ...
    function search_RegisterDTPBF_baru($search)
    {
        ...
        $this->dt->select('transaksi_farmasi_terima_pbf_detail.code, 
        transaksi_farmasi_terima_pbf_detail.name, 
        transaksi_farmasi_terima_pbf_detail.batchnumber, 
        transaksi_farmasi_terima_pbf_detail.uom,
        transaksi_farmasi_terima_pbf_detail.documentdate,
        transaksi_farmasi_terima_pbf_detail.relationname, 
        transaksi_farmasi_terima_pbf_detail.referencenumber, 
        transaksi_farmasi_terima_pbf_detail.expireddate, 
        transaksi_farmasi_terima_pbf_detail.qtybox,
        transaksi_farmasi_terima_pbf_detail.volume, 
        transaksi_farmasi_terima_pbf_detail.qty, 
        transaksi_farmasi_terima_pbf_detail.uom, 
        obat.types, 
        transaksi_farmasi_terima_pbf_detail.price, 
        transaksi_farmasi_terima_pbf_detail.purchaseprice,
        transaksi_farmasi_terima_pbf_detail.subtotal,  
        transaksi_farmasi_terima_pbf_header.invoicedate,
        transaksi_farmasi_terima_pbf_detail.taxamount,
        transaksi_farmasi_terima_pbf_detail.totaldiscount');
    ...
    function search_realisasi_ranap($search)
    {

        $this->dt = $this->db->table('client_daftar_rawatinap');
        $this->dt->select('doktername, SUM(realcost)as realcost, SUM(realisasi_klaim)as realisasi, COUNT(doktername)as jumlah');


        $this->dt->where('tgl_realisasi >=', $search['mulai']);
        $this->dt->where('tgl_realisasi <=', $search['sampai']);
        $this->dt->where('tmo', 0);
        $this->dt->like('paymentmethodname', 'JKN');
        $this->dt->where('doktername <>', null);
        $this->dt->where('doktername <>', '');
        $this->dt->groupBy('doktername');
    ...
    function search_realisasi_rajal($search)
    {
    ...
    function search_realisasi_ranap_tmo($search)
    {
    ...
    function ambildataDistribusigudang()
    {
        $lokasi = session()->get('locationcode');
        $this->dt = $this->db->table('transaksi_farmasi_distribusi_detail');
    ...
    function search_RegisterDistribusigudang($search)
    {
        $lokasi = session()->get('locationcode');
        $this->dt = $this->db->table('transaksi_farmasi_distribusi_detail');
        $this->dt->where('documentdate >=', $search['mulai']);
    ...
    function search_detail_distribusi_baksos($journalnumber)
    {
    ...
    function search_header_pelayanan_rme_rajal($journalnumber)
    {
        $this->dt = $this->db->table('transaksi_farmasi_pelayanan_header');
        $this->dt->where('referencenumber', $journalnumber);
        $this->dt->whereIn('groups', ['RJ', 'IGD']);
    ...
    function search_detail_pelayanan_resep($journalnumber_resep)
    {
        $this->dt = $this->db->table('transaksi_farmasi_pelayanan_detail');
        $this->dt->where('journalnumber', $journalnumber_resep);
        $this->dt->orderBy('id');
        $query = $this->dt->get();
        return $query->getResultArray();
    }

    function search_header_pelayanan_pulang($journalnumber)
    {
        $this->dt = $this->db->table('transaksi_farmasi_pelayanan_header');
        $this->dt->where('journalnumber', $journalnumber);
    ...
    function search_detail_pelayanan_pulang($journalnumber)
    {
        $this->dt = $this->db->table('transaksi_farmasi_pelayanan_detail');
        $this->dt->where('journalnumber', $journalnumber);
        $this->dt->where('terapiPulang', 1);
