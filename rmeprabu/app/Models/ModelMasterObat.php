...
class ModelMasterObat extends Model
{
    ...
    protected $allowedFields = [
        ...
        'purchaseprice_ori', 'taxprice_ori', 'salesprice_ori', 'inactive', 'keterangan', 'sumber', 'margin', 'beban'
    ];
    ...
    function get_data_obat_baru($id)
    {
        $tb = "obat";
        $this->dt = $this->db->table('obat');
        $this->dt->select('code, uom, name, round(obat.purchaseprice)as hargasebelum, taxprice as purchaseprice, volume, manufacturename, round(obat.salesprice)as salesprice');
        $this->dt->where('id', $id);
        $this->dt->limit(1);
        $query = $this->dt->get();
        return $query->getRowArray();
    ...
    function get_data_obat_depo_sp($code, $locationcode)
    {
        $this->dt = $this->db->table('obat');
        $this->dt->join('obat_balance', 'obat_balance.code=obat.code', 'left');
        $this->dt->select('obat.code, obat.uom, SUM(round(obat_balance.balance,0))as stock, obat.name, SUM(obat.purchaseprice + obat.taxprice)as purchaseprice , obat_balance.locationcode');
    ...
    function search_stok_obat($search)
    {
        ...
        $this->dt->select('obat_balance.code, obat.uom, obat_balance.balance as stock, obat.name, obat_balance.expireddate, obat_balance.batchnumber, obat.manufacturename, obat.taxprice, obat.groups, obat.category, obat_balance.id, , obat_balance.balance_temp');
        $this->dt->where('obat_balance.locationcode', $search['locationcode']);
        $this->dt->where('obat_balance.expireddate >', '2023-01-01');
        // if ($search['locationcode'] != "") {
        //     $this->dt->like('obat_balance.locationcode', $search['locationcode']);
        // } else {
        ...
        if ($search['namaobat'] != "") {
            $this->dt->where('obat.code', $search['namaobat']);
        }
    ...
    function search_DataObat_Pesan($search)
    {
        ...
        $this->dt->join('obat_balance', 'obat_balance.code=obat.code', 'left');
        $this->dt->select('obat.code, obat.uom, SUM(obat_balance.balance)as stock, obat.name, obat.purchaseprice, obat_balance.locationcode');
        //$this->dt->where('obat_balance.code', '1000396');
        if ($search['codeobat'] != "") {
            $this->dt->like('obat_balance.code', $search['codeobat']);
        }
        if ($search['namaobat'] != "") {
    ...
    function caridataobatamprahruangan()
    {
        $this->dt = $this->db->table('obat');
        $this->dt->join('obat_balance', 'obat_balance.code=obat.code', 'left');
        $this->dt->select('obat.code, obat.uom, SUM(obat_balance.balance)as stock, obat.name, obat.id, obat.volume');
        $this->dt->groupBy('obat.code');
        $this->dt->orderBy('name', 'ASC');
        $this->dt->limit(1);
        $query = $this->dt->get();
    ...
    function search_DataObat_amprah_ruangan_asli($search)
    {
        $this->dt = $this->db->table('obat');
        $this->dt->join('obat_balance', 'obat_balance.code=obat.code', 'left');
        $this->dt->select('obat.code, obat.uom, SUM(obat_balance.balance)as stock, obat.name, obat.purchaseprice, obat_balance.locationcode');
    ...
    function search_DataObat_amprah_ruangan_farmasi($search)
    {
        $this->dt = $this->db->table('obat_balance');
        $this->dt->join('obat', 'obat.code=obat_balance.code', 'left');
        $this->dt->select('obat.code, obat.uom, SUM(obat_balance.balance)as stock, obat.name, obat.purchaseprice, obat_balance.locationcode');
        ...
        $this->dt->where('obat_balance.locationcode', 'FARMASI');
        //$this->dt->orlike('obat_balance.locationcode', 'KONSINYASI');
        $this->dt->groupBy('obat.code');
        $this->dt->limit(1000);
        $query = $this->dt->get();
        return $query->getResultArray();
    ...
    function get_data_obat_depo_sp_ruangan_baru($code, $locationcode, $tanda)
    {
        $this->dt = $this->db->table('obat');
        $this->dt->join('obat_balance', 'obat_balance.code=obat.code', 'left');
        ...
        if ($tanda == 1) {
            ...
            $this->dt->select('obat.code, obat.uom, 0 as stock, obat.name, obat.purchaseprice, obat_balance.locationcode');
            $this->dt->where('obat_balance.locationcode', 'FARMASI');
    ...
    {
        $this->dt = $this->db->table('obat');
        $this->dt->join('obat_balance', 'obat_balance.code=obat.code', 'right');
        ...
        //$this->dt->where('obat.code', '1003586');
        if ($search['codeobat'] != "") {
            $this->dt->like('obat_balance.code', $search['codeobat']);
        }
        if ($search['namaobat'] != "") {
    ...
    function search_DataObat_pelayanan_baru($search)
    {
        $this->dt = $this->db->table('obat');
        $this->dt->join('obat_balance', 'obat_balance.code=obat.code', 'left');
        $this->dt->select('obat_balance.code, obat_balance.uom, SUM(obat_balance.balance)as stock, obat.name, obat.id');
        ...
        $this->dt->where('obat_balance.locationcode', $search['lokasi']);
        $this->dt->orderBy('obat.name');
        $this->dt->limit(100);
    ...
    function search_stok_obat2023($search)
    {
        $this->dt = $this->db->table('obat_balance');
        $this->dt->join('obat', 'obat.code=obat_balance.code', 'inner');
        $this->dt->select('obat_balance.code, obat.uom, SUM(obat_balance.balance) as stock, obat.name, obat_balance.expireddate, obat_balance.batchnumber, obat.manufacturename, obat.taxprice, obat.groups, obat.category');
        $this->dt->where('obat_balance.locationcode', $search['locationcode']);
        $this->dt->where('obat_balance.expireddate >', '2023-01-01');
        // if ($search['locationcode'] != "") {
        //     $this->dt->like('obat_balance.locationcode', $search['locationcode']);
    ...
    function search_stok_obat2023_ruangan($search)
    {
        $this->dt = $this->db->table('obat_balance');
        $this->dt->join('obat', 'obat.code=obat_balance.code', 'inner');
        $this->dt->select('obat_balance.code, obat.uom, SUM(obat_balance.balance) as stock, obat.name, obat_balance.expireddate, obat_balance.batchnumber, obat.manufacturename, obat.taxprice, obat.groups, obat.category');
        $this->dt->where('obat_balance.locationcode', session()->get('locationcode'));
        $this->dt->where('obat_balance.expireddate >', '2023-01-01');
        // if ($search['locationcode'] != "") {
        //     $this->dt->like('obat_balance.locationcode', $search['locationcode']);
        // } else {
    ...
    function caridataobatbaru($lokasi)
    {
        $this->dt = $this->db->table('obat_balance');
        $this->dt->join('obat', 'obat.code=obat_balance.code', 'left');
        $this->dt->select('obat_balance.code, obat_balance.uom, SUM(obat_balance.balance)as stock, obat.name, obat.id');
        $this->dt->where('obat_balance.locationcode', $lokasi);
        $this->dt->groupBy('obat_balance.code');
        $this->dt->orderBy('name', 'ASC');
        //$this->dt->limit(1);
        $query = $this->dt->get();
    ...
    function caridataobatbaruinput($lokasi, $code, $namaobat)
    {

        $this->dt = $this->db->table('obat_balance');
        $this->dt->join('obat', 'obat.code=obat_balance.code', 'left');
        $this->dt->select('obat.id, obat.code, obat.uom, SUM(round(obat_balance.balance,0))as stock, obat.name, obat.purchaseprice, obat_balance.locationcode, obat.salesprice');
        $this->dt->like('obat.code', $code);
        $this->dt->like('obat.name', $namaobat);
        $this->dt->where('obat_balance.locationcode', $lokasi);
        $this->dt->groupBy('obat_balance.code');
        //$this->dt->limit(1);
        $query = $this->dt->get();
    ...
    function search_DataObat_barang_masuk_faktur($search)
    {


        $this->dt = $this->db->table('obat');
        $this->dt->join('obat_balance', 'obat.code=obat_balance.code', 'left');
        $this->dt->select('obat.id, obat.code, obat.uom, SUM(obat_balance.balance)as stock, obat.name, obat.purchaseprice, obat_balance.locationcode');
        //$this->dt->like('obat.locationcode', 'FARMASI');
        //$this->dt->where('obat_balance.code', '1000396');
    ...
    function search_DataObat_barang_masuk_faktur_vitual($search)
    {
        ...
        $this->dt->like('obat_balance.locationcode', 'VITUAL');
        $this->dt->groupBy('obat.code');
        $this->dt->limit(1000);
        $query = $this->dt->get();
        return $query->getResultArray();
    ...
    function get_list_BacthNumber_distribusi_vitual_baru($code, $locationcode)
    {
        $tb = "obat";
        $this->dt = $this->db->table('obat_balance');
        $this->dt->select('id, code, uom, batchnumber, expireddate, balance, createddate');
        $this->dt->where('code', $code);
        //$this->dt->where('locationcode', $locationcode);
        $this->dt->where('locationcode', 'VITUAL');
        $this->dt->where('balance >', 0);
        $this->dt->orderBy('expireddate', 'ASC');
        $this->dt->limit(100);
        $query = $this->dt->get();
    ...
    function get_list_BacthNumber_distribusiGC($code, $locationcode)
    {
        $tb = "obat";
        $this->dt = $this->db->table('obat_balance');
        $this->dt->select('id, code, uom, batchnumber, expireddate, balance, createddate');
    ...
    function search_DataObat_barang_akan_retur($search)
    {
        ...
        $this->dt->select('transaksi_farmasi_terima_pbf_detail.id, transaksi_farmasi_terima_pbf_detail.code, transaksi_farmasi_terima_pbf_detail.name, transaksi_farmasi_terima_pbf_detail.relation, 
        transaksi_farmasi_terima_pbf_detail.relationname, transaksi_farmasi_terima_pbf_detail.qtybox, transaksi_farmasi_terima_pbf_detail.volume, transaksi_farmasi_terima_pbf_detail.subtotal,
        transaksi_farmasi_terima_pbf_header.receiptdate, transaksi_farmasi_terima_pbf_detail.uom, transaksi_farmasi_terima_pbf_detail.batchnumber, transaksi_farmasi_terima_pbf_detail.referencenumber');
        //$this->dt->like('obat.locationcode', 'NONE');
        $this->dt->where('transaksi_farmasi_terima_pbf_detail.relation', $search['kodesuplier']);
        if ($search['codeobat'] != "") {
            $this->dt->like('transaksi_farmasi_terima_pbf_detail.code', $search['codeobat']);
        }
        if ($search['namaobat'] != "") {
        ...
        //$this->dt->groupBy('obat.code');
        $this->dt->orderBy('transaksi_farmasi_terima_pbf_detail.id', 'DESC');
        $this->dt->limit(1000);
        $query = $this->dt->get();
        return $query->getResultArray();
    }
    ...
    function get_data_obat_retur($id)
    {
        $tb = "transaksi_farmasi_terima_pbf_detail";
        $this->dt = $this->db->table('transaksi_farmasi_terima_pbf_detail');
        //$this->dt->select('code, uom, name, round(obat.purchaseprice)as hargasebelum, taxprice as purchaseprice, volume, manufacturename');
        $this->dt->where('id', $id);
        $this->dt->limit(1);
        $query = $this->dt->get();
        return $query->getRowArray();
    ...
    function get_data_obat_depo_sp_vitual($code, $locationcode)
    {
        $this->dt = $this->db->table('obat');
        $this->dt->join('obat_balance', 'obat_balance.code=obat.code', 'left');
        $this->dt->select('obat.code, obat.uom, SUM(round(obat_balance.balance,0))as stock, obat.name, SUM(obat.purchaseprice + obat.taxprice)as purchaseprice , obat_balance.locationcode');
    ...
    function get_data_obat_depo_sp_vitual_kosong($code)
    {
        $this->dt = $this->db->table('obat');
        $this->dt->join('obat_balance', 'obat_balance.code=obat.code', 'left');
        $this->dt->select('obat.code, obat.uom, SUM(round(obat_balance.balance,0))as stock, obat.name, SUM(obat.purchaseprice + obat.taxprice)as purchaseprice , obat_balance.locationcode');
    ...
    function get_data_obat_vitual($search)
    {
        $this->dt = $this->db->table('obat');
        $this->dt->join('obat_balance', 'obat.code=obat_balance.code', 'left');
        $this->dt->select('obat.id, obat.code, obat.uom, SUM(obat_balance.balance)as stock, obat.name, obat.purchaseprice, obat_balance.locationcode');
    ...
    function get_list_BacthNumber_distribusi_vitual_baru_vitual($code, $locationcode)
    {
        $tb = "obat";
        $this->dt = $this->db->table('obat_balance');
        $this->dt->select('id, code, uom, batchnumber, expireddate, balance, createddate');
    ...
    function get_list_BacthNumber_distribusi_konsinyasi($code, $locationcode)
    {
        $tb = "obat";
        $this->dt = $this->db->table('obat_balance_konsinyasi');
        $this->dt->select('id, code, uom, batchnumber, expireddate, balance, createddate');
        $this->dt->where('code', $code);
        //$this->dt->where('locationcode', $locationcode);
    ...
    function get_data_obat_BN_distribusi_konsinyasi($id)
    {
        $tb = "obat_balance_konsinyasi";
        $this->dt = $this->db->table('obat_balance_konsinyasi');
        $this->dt->select('id, code, uom, batchnumber, expireddate, round(balance)as balance, createddate');
        $this->dt->where('id', $id);
        $this->dt->limit(1);
    ...
    function get_list_BacthNumber_distribusi_konsinyasi_real($code)
    {
        $tb = "obat";
        $this->dt = $this->db->table('obat_balance');
        $this->dt->select('id, code, uom, batchnumber, expireddate, balance, createddate');
    ...
    function get_list_BacthNumber_distribusi_hibah($code, $locationcode)
    {
        $tb = "obat";
        $this->dt = $this->db->table('obat_balance');
        $this->dt->select('id, code, uom, batchnumber, expireddate, balance, createddate');
