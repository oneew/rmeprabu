...
class ModelrajalOrderRad extends Model
{
    ...
    function ambildatarajalHD()
    {
        $this->dt = $this->db->table($this->table);
        $this->dt->orderBy('documentdate', 'DESC');
        $this->dt->where('poliklinikname', 'Hemodialisa');
        $this->dt->limit(400);
        $query = $this->dt->get();
        return $query->getResultArray();
    }

    function ambildatarajalRHM()
    {
        $this->dt = $this->db->table('transaksi_pelayanan_daftar_rawatjalan');
        $this->dt->orderBy('documentdate', 'DESC');
        //$this->dt->where('poliklinikname', 'REHABILITASI MEDIK (FISIOTERAPHY)');
        $this->dt->limit(1000);
        $query = $this->dt->get();
        return $query->getResultArray();
    }
