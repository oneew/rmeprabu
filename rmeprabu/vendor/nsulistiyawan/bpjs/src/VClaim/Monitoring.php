...
class Monitoring extends BpjsService
{
    ...
    public function getListFingerPrint($tglMulai)
    {
        $response = $this->get('SEP/FingerPrint/List/Peserta/TglPelayanan/' . $tglMulai);
        return json_decode($response, true);
    }

    public function getDataFingerPrint($nokartu, $tglMulai)
    {
        $response = $this->get('SEP/FingerPrint/Peserta/' . $nokartu . '/TglPelayanan/' . $tglMulai);
        return json_decode($response, true);
    }
}
